## 18.2 Claude Agent Team深度集成指南

在2026年，Claude Agent Team框架已成为构建协作智能体系统的最佳实践。本章深入讲解如何在OpenClaw中集成Claude Agent SDK，实现多Agent的协作编排和任务分解。

### 18.2.1 Claude Agent SDK核心概念
...
│  (User Interface / Channel Integration)         │
├─────────────────────────────────────────────────┤
│       Claude Agent Team Orchestration           │
│  (Agent Team Manager / Task Router)             │
├─────────────────────────────────────────────────┤
│       Claude Agent SDK Foundation               │
│  (Agent Executor / MCP Client / Model API)      │
├─────────────────────────────────────────────────┤
│       Infrastructure Layer                       │
│  (Model Provider / MCP Servers / Tools)         │
└─────────────────────────────────────────────────┘
```

### 18.2.2 Agent Team架构设计
...
class AgentProfile:
    ...
    role: AgentRole
    system_prompt: str
    model: str                        # "claude-3-5-sonnet-20250514"
...
- 清晰性：是否易于理解
- 专业性：是否符合行业标准""",
            model="claude-3-5-haiku-20250514",  # 验证任务用轻量模型
  ...
  "openclaw": {
    "agents": {
      ...
      "team_manager": {
        ...
        "default_model": "claude-3-5-sonnet-20250514",
        "max_team_size": 10,
        "agent_timeout": 300,
...
class AgentIsolationManager:
    ...
    def _estimate_tokens(self, task: Dict) -> int:
        ...
        return len(content) // 4 + 500  # 加上system prompt的估算
