...
- **SSRF 防护**：当前浏览器配置采用受信网络默认模型，`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork` 默认允许私有网络访问；如需严格阻止内网/私有地址，应显式设为 `false`，再按需配置 hostname allowlist。
...
在安全边界之上，通过工具策略控制哪些智能体可以使用浏览器相关工具，拒绝规则优先于允许规则。还要补上一层插件事实：当前 browser 作为 bundled plugin 提供；如果你启用了 `plugins.allow` 白名单，就必须显式允许 `browser`，否则单独打开 `browser.enabled=true` 也无法恢复被插件白名单裁掉的浏览器能力。详见 [5.2 工具策略：允许、拒绝与分层策略](5.2_tool_policy.md)、官方工具文档 [https://docs.openclaw.ai/tools](https://docs.openclaw.ai/tools) 与 [Browser 文档](https://docs.openclaw.ai/tools/browser)。
...
| 层级 | 能力 | 适用场景 | 依赖 | 性能与成本 |
| --- | --- | --- | --- | --- |
| **L0** | 搜索引擎 + 网页抓取 | 日常信息检索（覆盖 80% 场景） | `web_search`（Brave、DuckDuckGo、Exa、Firecrawl、Gemini、Grok、Kimi、MiniMax、Ollama、Perplexity、SearXNG、Tavily 等 provider）+ `web_fetch` | 最低 |
...
以下命令对应当前官方 CLI 文档中的常见浏览器生命周期入口；实际操作前仍建议先以本地 `openclaw --help`、相关子命令 `--help` 和 Tool Catalog 为准。[浏览器工具参考](https://docs.openclaw.ai/tools/browser)。
...
当前更准确的表述是：`web_fetch` 会执行 HTTP GET，并优先用 Readability 提取正文；它并不是一个独立的“Readability 工具”。如遇 SPA 页面、登录态页面或抽取失败，可考虑引入支持 JS 渲染的抓取服务（如 Firecrawl）作为补充或 fallback。使用任何第三方抓取工具时，务必遵守目标网站的服务条款。
