...
> [!WARNING]
> **通配符脚枪与 `elevated` 执行特权**：在 `allow` 列表中过度使用 `*`（尤其是配合 `tools.elevated` 绕过沙箱提权时）意味着对所有未知插件无条件授权，极易造成权限静默放大。建议必须严控 `allowFrom`，避免 `*`，并总是将验证命令（如 `security audit`）纳入发布前检查。
>
...
当主智能体通过 `sessions_spawn` 派生子智能体时，子智能体的工具可用范围会被**自动收窄**。这是一个源码层面的硬编码安全边界（`pi-tools.policy.ts`），即使配置中没有显式声明 deny，以下工具也会被系统级禁用。
