...
官方配置可拆成四个关键块，参考：https://docs.openclaw.ai/zh-CN/tools 与 https://docs.openclaw.ai/zh-CN/gateway/configuration#tools。
...
1. **模型判定** ：模型推理后输出工具调用意图 `shell.exec`，参数为 `kubectl delete pod error-pod-xxx -n staging`。
2. **策略校验** ：运行时检查全局 `tools.deny` 列表，发现 `shell.*` 命中 deny 规则。
...
- **外部客服群** ：作为基础配置，只允许使用 `kb_search`（知识库搜索）解答问题。
- **内部研发群** ：除了知识库，对于特定的管理员账号（如 `123456789`），允许通过 `toolsBySender` 放开 `shell.*` 这类命令执行工具来运行受限脚本；其他研发成员依然受基础策略管控。
...
```json5
{
  ...
  channels: {
    ...
    telegram: {
      groups: {
        '*': {
          tools: { deny: ['shell.*', 'fs.write*'] },
          toolsBySender: {
            '123456789': { alsoAllow: ['shell.*'] },
          },
        },
      },
    },
...
- Tools：https://docs.openclaw.ai/zh-CN/tools
- Config tools：https://docs.openclaw.ai/zh-CN/gateway/configuration#tools
