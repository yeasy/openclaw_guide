...
### 5.2.1 官方工具策略结构：五个核心块

官方配置可拆成五个关键块：

- `tools.allow`：全局允许列表，支持通配符；用于限制候选集合，不适合作为“顺手启用插件”的增量开关。
- `tools.alsoAllow`：在 `tools.profile` 基础上增量放行工具或插件工具。
- `tools.deny`：全局拒绝列表，支持通配符，且优先级高于 allow。
- `tools.profile`：选择默认工具配置文件（`minimal`/`coding`/`messaging`/`full`）。
- `channels.*.groups.*.tools`：按渠道与群组分层限制工具（含 `allow`/`alsoAllow`/`deny` 与 `toolsBySender`）。
...
| 全局增量放行 | `tools.alsoAllow` | `[]` | 在 profile 基础上添加工具，常用于启用可选插件工具 |
| 全局允许清单 | `tools.allow` | `[]` | 限制性 allowlist；在严格配置下需显式声明完整候选集合 |
...
| `coding` | `group:fs`、`group:runtime`、`group:web`、`group:sessions`、`group:memory`、`cron`、`image`、`image_generate`、`music_generate`、`video_generate`，以及已配置的 `bundle-mcp` |
...
在评估一次工具调用是否被允许时，更稳妥的理解方式是：先构造**全局基础集合**，再由群组 / 发送者规则继续收窄，而不是把所有 `allow` / `deny` 理解成可以任意相互覆盖。

| 优先级 | 规则层 | 说明 |
| --- | --- | --- |
| 1（最高） | `tools.deny` | 全局硬拒绝；后续群组或发送者规则不能把它重新放开 |
| 2 | `tools.profile` | 构成基础工具集合 |
| 3 | `tools.alsoAllow` | 在基础集合上增量加入可选工具或插件工具 |
| 4 | `tools.allow` | 如果设置为限制性 allowlist，应包含完整候选集合；不要用它替代 `alsoAllow` |
| 5 | `channels.*.groups.*.tools.deny` | 在群组层继续收窄 |
| 6 | `channels.*.groups.*.tools.allow` / `alsoAllow` / `toolsBySender` | 只在群组层内做更细粒度裁剪，不能覆盖全局 deny |

一条经验法则：**拒绝规则永远优先于允许规则；想在 profile 基础上增加工具时优先用 `alsoAllow`；group / sender 的 `alsoAllow` 只能放开群组层默认禁用项，不能覆盖全局或 agent 级 deny。**
...
- 增量启用：需要把 `browser`、`lobster` 等可选能力加到 `coding` profile 时，优先使用 `tools.alsoAllow`；只有确实要进入限制性 allowlist 模式时才使用 `tools.allow`。同一个 policy scope 里不要同时设置非空 `allow` 与 `alsoAllow`，当前 schema 会拒绝这种混用。
- 插件加载：`tools.alsoAllow` 只能在工具策略阶段放行工具，不能替代 `plugins.allow` 或插件启用配置；浏览器、Lobster 等插件工具仍需先被插件 allowlist 加载。

> [!WARNING]
> **通配符滥用**：在 `allow` 列表中过度使用 `*` 意味着对所有未知工具与插件无条件授权，容易导致权限被静默放大。建议严控 `tools.allow` 与 `plugins.allow` 的范围，避免 `*`，并总是将验证命令（如 `security audit`）纳入发布前检查。

> [!NOTE]
> **`tools.elevated` 不是工具放行机制，而是 `exec` 专用的宿主机逃逸口**。当智能体运行在沙箱中时，`elevated` 允许 `exec` 命令在宿主机而非容器内执行，但它**不授予额外工具访问权**——工具的允许/拒绝仍完全由上述优先级表决定。`elevated` 支持全局配置（`tools.elevated.*`）和按智能体配置（`agents.list[].tools.elevated.*`），`allowFrom` 是按渠道组织的发送者 allowlist（如 `discord`、`whatsapp`），不是模型 provider 限制。provider 维度的工具控制应使用 provider policy。详见 [Elevated Mode](https://docs.openclaw.ai/tools/elevated)。
...
| 工具 | 禁用原因 |
|------|---------|
| `subagents` / `sessions_spawn` | 叶子节点不能再派生下级 |
...
**配置覆盖**：当前配置面是 `tools.subagents.tools.allow` / `tools.subagents.tools.alsoAllow` / `tools.subagents.tools.deny`。其中 `allow` 一旦设置就是 allow-only，必须列出子智能体完整可用工具集合；`alsoAllow` 用于从子智能体默认禁用项中恢复少量工具；若只是收窄权限，优先使用 `deny`。当前 schema 不允许同一层同时设置非空 `allow` 与 `alsoAllow`。
...
只有确实需要持久在线会话间通信时，才考虑用 `tools.subagents.tools.alsoAllow: ["sessions_send"]` 恢复该工具；子智能体结果应优先通过 announce 协议回传。
