...
同样，你也可以通过 CLI 或 ClawHub 发现、安装和调用别人建好的能力包。三个动作有明确的分界：

```bash
# 1. 发现：语义搜索匹配的技能
openclaw skills list --eligible "daily report"
# 或通过 ClawHub 搜索
openclaw clawhub search "daily report"
...
> **⚠️ 以下为假设性教学场景，用于说明供应链风险的攻击面**
>
> 从 ClawHub 下载技能包时，必须正视其背后的供应链风险。公共技能仓库面临与 npm/PyPI 类似的供应链攻击：恶意技能可能伪装成常用工具，在 `SKILL.md` 中嵌入诱导性指令。由于技能本质上是 Markdown 文件，大模型容易被精心设计的 prompt 引导而不假思索地执行其中的操作。
