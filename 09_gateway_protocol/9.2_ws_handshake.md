...
### 9.2.5 源码级细节：三层鉴权、限流与渠道健康监控

官方 Gateway 的鉴权远不止"验一个 token"，源码（`gateway/auth.ts`、`device-auth.ts`、`connection-auth.ts`）实现了三层独立鉴权 + 多维限流 + 渠道健康监控的纵深架构。

**三层鉴权架构**：

| 层级 | 源码文件 | 职责 | 细节 |
|------|----------|------|------|
| **设备层** | `device-auth.ts` | 设备身份建立 | v2/v3 版本化载荷，含平台与设备族元数据，密码学签名（时间戳 + nonce） |
| **连接层** | `connection-auth.ts` | 连接凭据解析 | 从配置/环境变量/密钥输入多源解析，支持令牌/密码优先级模式与回退 |
| **用户层** | `auth.ts` | 用户身份验证 | 四种认证模式：`token`、`password`、`trusted-proxy`、`none`；支持 Tailscale 身份、可信代理头提取 |

`authorizeGatewayConnect()` 的完整鉴权链路：解析客户端 IP（代理感知）→ 检查限流器 → 尝试 Tailscale 认证 → 尝试令牌认证（安全时间常量比较） → 尝试密码认证 → 尝试可信代理认证 → 记录失败或重置限流计数器。
...
控制面还有独立的写保护限流（`control-plane-rate-limit.ts`）：每设备每分钟最多 3 次写操作，按 `{deviceId}|{clientIp}` 分桶。

**渠道健康监控**（`channel-health-monitor.ts` + `channel-health-policy.ts`）：
