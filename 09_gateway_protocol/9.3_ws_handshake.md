...
当前 Control UI / WebChat 的握手不再是旧资料里常见的 `pairing_token -> session_token -> HELLO/HELLO_ACK` 两阶段流程，而是三阶段：（1）WebSocket Upgrade 建立传输层连接，（2）Gateway 发出 **`connect.challenge`** 事件携带 `nonce`，（3）客户端用 `nonce` 签名后发送 `connect` RPC 完成认证。只有第三步通过后，连接才真正进入可用状态。
