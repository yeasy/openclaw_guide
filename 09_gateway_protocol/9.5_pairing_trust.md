...
sequenceDiagram
    ...
    alt 已配对 / 本地自动批准 / bootstrap 已获批准或受信 handoff
      Trust-->>Gateway: ✓ 通过
      Gateway-->>UI: [7] hello-ok
    else 新设备且尚未批准
      Trust-->>Gateway: 需要配对批准
      Gateway-->>Operator: [7] 广播 device.pair.requested
      Gateway-->>UI: [8] NOT_PAIRED / PAIRING_REQUIRED + close
    end
```
...
| DM 临时码 / 配对码 | 渠道用户身份引导，例如控制“谁可以和机器人私聊” | 渠道侧 sender 配对能力，不是远端节点或 Control UI 设备身份的默认启动路径，也不会替代 `session.dmScope` 的会话隔离配置 |
