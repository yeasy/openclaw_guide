...
sequenceDiagram
    ...
    alt 已配对 / 本地自动批准 / 有效 bootstrap
      ...
      Gateway-->>UI: [8] 广播 device.pair.requested
    end
```
...
早期资料里常见的 **DM 配对**（Direct Message Pairing）并不是完全无效，但它解决的是“谁可以通过外部渠道和 bot 聊天”，不是当前 Control UI 设备握手的默认主路径。阅读旧资料时，务必区分下面两类流程：

| 流程 | 更适合的场景 | 当前应如何理解 |
|------|-------------|----------------|
| Control UI 设备身份 + challenge-response | 本地浏览器、WebChat、控制台访问 | 当前控制面的主路径；未配对的新设备会先进入 pending pairing，批准后再重连 |
| DM 临时码 / 配对码 | 外部渠道 sender 访问控制、跨渠道人工引导 | 解决“谁能给 bot 发消息”，不等于当前 WS 设备握手 |
