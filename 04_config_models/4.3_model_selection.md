...
| 数据分析助手 | 高（复杂推理） | 中（可等 30 秒） | 中 | 大模型为主（如 `openai/gpt-5.4` 或 `anthropic/claude-sonnet-4-6`），关注上下文窗口 |
| 定时巡检报告 | 高（结构化输出） | 高（离线异步） | 低 | 大模型为主，失败后自动重试而非降级 |

以此表为参考，客服场景中，把当前目录里可用的低延迟模型设为 `primary`，并把 `openai/gpt-5.4` 或 `anthropic/claude-sonnet-4-6` 放进 `fallbacks`，能在控制成本的同时保证复杂问题不崩；而内部分析场景则相反，应把大模型作为主力。
