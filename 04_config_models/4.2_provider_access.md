...
真正的调用目标由 `provider/model` 形式的模型标识决定，例如：`openai/gpt-5.4`、`anthropic/claude-sonnet-4-6`。
...
环境变量：`OPENAI_API_KEY`
常用模型标识：`openai/gpt-5.4`、`openai/gpt-5.4-mini`、`openai/gpt-5.4-nano`
...
**3. Anthropic Vertex（通过 Google Cloud Vertex AI 调用 Claude）**
