...
模型标识统一使用 `provider/model` 格式，斜杠前的 `provider` 必须能在 `models.providers`（自定义供应商）或认证档案（内置供应商）中找到匹配。常见的命名形态（以下名称仅为格式示例，实际可用模型以各供应商官方发布为准）：

- `openai/gpt-5.4`、`openai/gpt-mini`、`openai/gpt-nano`
