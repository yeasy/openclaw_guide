...
如果你走的是 ChatGPT/Codex 订阅 OAuth，当前配置应使用 `openai-codex/*` 模型引用，并保留 `openai-codex` auth profile；直接 OpenAI Platform API Key 路径才使用 `openai/*`。关键不是复用某段历史配置，而是**要和当前模型目录、auth profile 与运行时选择保持一致**。这样即便主链路因限流而暂时不可用，请求也能自动降级到次级模型或备用供应商，保证服务连续性。
