...
- **内容规则校验**：运行 `python check_project_rules.py -v` 确保您的 Markdown 符合格式要求（如中文全角引号、加粗文字和前后汉字的间距、文件结尾必须有换行符、章节数字的正确格式等）。
- **Gitbook 构建与死链检查**：运行 `npm run build` 或 `python scripts/check_links.py` （如果有的话），确保本地编译正常，没有内部死链。

### 2. Markdown 格式约定

- **JSON 与 JSON5**：
  - 如果代码块为标准的 JSON（可用于 `JSON.parse()` 的无注释、强引号、无尾逗号），请使用 ````json````。
  - **重要约定**：但凡配置示例中带有 `// 注释`、使用无引号 Key、存在尾部逗号等，**请严格使用 ````json5````** 标签，防止部分 Markdown 渲染器出现解析警告。由于 OpenClaw 使用 JSON5，大部分配置示例都会是 JSON5。
- **引用格式**：
...
3. **PR 描述**：
   ...
   - 对格式调整的 PR，请说明是否已经在本地通过 `check_project_rules.py` 的校验。
