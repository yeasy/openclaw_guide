...
- **Honkit 构建验证**：运行 `npm run build` 确保本地编译正常，没有内部死链。

### 2. Markdown 格式约定

- **JSON 与 JSON5**：
  - 如果代码块为标准的 JSON（可用于 `JSON.parse()` 的无注释、强引号、无尾逗号），请使用 ````json````。
  - **重要约定**：但凡配置示例中带有 `// 注释`，**请使用 ````jsonc````** 标签（JSON with Comments）。`json5` 标签在 honkit 构建链中不被支持，会导致高亮告警。如果配置示例使用了无引号 Key 或尾部逗号等 JSON5 特性，建议改写为标准 JSON 并将注释放在 `jsonc` 代码块中。
- **引用格式**：
