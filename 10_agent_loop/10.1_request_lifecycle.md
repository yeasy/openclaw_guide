...
1. **步骤 ①②③**：消息到达，Gateway 验证身份与配对，按路由把请求交给对应 Node 下的 Agent。这些是第九章控制平面的职责范围。
2. **步骤 ④⑤⑥⑦**：Agent 从 Session 提取历史记忆，拼装提示词，发送给模型。这里有超时与重试控制——对应本章 10.3–10.4 的内容。
3. **步骤 ⑧⑨⑩⑪**：模型推理后决定调用工具（如搜索），执行完后把结果回传给模型。此阶段受故障降级策略保护——对应本章 10.5 的 Suspend/Resume 机制。
...
有了这张全局图，接下来 10.2 将深入 π 运行底座，解释为什么这套看似简单的事件-状态机架构能支撑复杂的长程智能体任务。
