...
为了支持企业级业务中动辄横跨十几小时、需要多人介入的业务流水线，OpenClaw 没有使用同步阻塞式的开发范式。当前嵌入式 agent 运行时会先按 `embeddedHarness.runtime` 选择已注册的 harness；默认 `auto` 模式允许插件 harness 接管匹配的 provider/model，未命中时才把 Pi 作为兼容 fallback（除非显式设置 `fallback: "none"`）。

### 10.2.1 引擎与外壳：Pi 与 OpenClaw 的定位

如果说 OpenClaw 是一个连接了各大聊天软件（如 WhatsApp、Telegram 等）、能 24 小时在线处理杂务的“全能管家外壳（Wrapper）”，那么 **Pi 是默认兼容路径中的底层执行引擎之一**；当 Codex 等插件 harness 被显式选择或在 `auto` 模式下成功声明支持时，实际执行路径会由对应 harness 接管。
...
- **流式 JSON 解析容错**：在对接不同提供商时，模型输出的工具调用参数极易出现有效 JSON 数据与截断垃圾字符拼凑的情况。引入宽容的流式 JSON 解析策略（如 `parseStreamingJson`），避免因尾部噪音导致整次调用失败。
- **可恢复错误的回注模型**：对于动辄运行数小时的自动化任务，将提供商端的网络闪断或短暂解析失败视为致命错误是不可接受的。更健壮的设计是将这些非逻辑性异常分类为“可恢复错误”，转化为一条消息回注给模型，让模型通过上下文自省能力尝试调整策略。
