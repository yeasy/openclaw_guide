...
flowchart TD
  ...
  p -->|"成功"| ok["正常返回"]
  p -->|"限流/失败"| f1["fallback 1: openai/gpt-mini"]
  f1 -->|"成功"| ok
...
两层机制的关系：auth profile 冷却（[11.2](11.2_rotation_cooldown.md)）在供应商内部轮换密钥/profile；模型 fallback 在模型之间跨级切换。先走 auth 层轮换，全部 profile 耗尽后才触发模型级 fallback。
