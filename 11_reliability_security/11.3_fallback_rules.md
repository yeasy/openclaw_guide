...
```mermaid
flowchart TD
   req["模型调用请求"] --> p["primary: openai/gpt-5.5"]
  p -->|"成功"| ok["正常返回"]
   p -->|"限流/失败"| f1["fallback 1: openai/gpt-5.4-mini"]
  f1 -->|"成功"| ok
...
两层机制的关系：auth profile 冷却（[11.2](11.2_rotation_cooldown.md)）会优先在同一 provider 内轮换 profile；模型 fallback 才是在模型之间跨级切换。但“先走 auth 层轮换”不等于“必须把所有 profile 都耗尽”：对于 overloaded / rate-limit 这类瞬时错误，系统会先消耗 `overloadedProfileRotations` / `rateLimitedProfileRotations` 预算，预算耗尽后就可以继续进入模型级 fallback。OpenAI/Codex 订阅 OAuth 也应优先保持 `openai/*` 模型引用，并用 auth profile 与运行时策略表达凭据和执行路径。
...
1. 基线通过：`models status --check` 没有认证缺失/过期，`models status --probe` 的 live auth 探针正常。
