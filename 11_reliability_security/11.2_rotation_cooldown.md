...
### 11.2.4 源码级细节：回退链路中的智能行为

从源码（`model-fallback.ts`、`auth-profiles/usage.ts`）中可以发现几个文档未充分说明的重要行为：

**上下文溢出错误不触发回退。** `isLikelyContextOverflowError()` 会检测 `request_too_large`、`context_window_exceeded` 等模式。一旦识别为上下文溢出，系统直接抛出错误而不尝试回退——因为更小的备选模型只会更快失败。此时正确的做法是触发压缩（Compaction）而非换模型。
...
**供应商级旁路。** 源码中 OpenRouter 和 Kilocode 两个供应商完全跳过 auth 冷却检查（`isAuthCooldownBypassedForProvider()`）。这意味着使用这些聚合服务时，冷却策略不会生效——因为它们自身有内部的供应商轮转机制。
