...
flowchart LR
  ...
  C --> D["探针验证<br/>models status --check / --probe"]
...
2. **验证**：用 `openclaw models status --check`、`openclaw models status --probe` 和 `openclaw health --json` 确认新 key 可用。
...
2. **探针证据**：`models status --check`、`models status --probe`、`health --json` 是否正常。
