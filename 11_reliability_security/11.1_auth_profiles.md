...
官方文档指出：当智能体在多 key 场景中选择了某把 key，这个选择可能会被写入智能体的认证档案文件，以便后续复用与审计。该文件位于：`~/.openclaw/agents/<agentId>/agent/auth-profiles.json`。参考：https://docs.openclaw.ai/tools/multi-agent-sandbox-tools。
