...
2. **第二层——沙箱隔离**：即使工具策略配置有误导致 `exec` 未被拒绝，也只有在该渠道命中了启用沙箱的智能体、未允许 elevated 逃逸、未挂载 Docker socket 或高危宿主目录、且 `workspaceAccess` / bind mounts 被正确收敛时，破坏才主要被限制在沙箱工作区。沙箱不是绝对安全边界；若关闭沙箱、开放宿主机执行或把宿主路径挂进容器，`rm -rf /` 这类命令仍可能影响宿主机或真实工作区。
...
工具治理的第一步不是写很长的提示词，而是把能力边界写进配置。官方工具治理支持 `allow/deny`、按渠道/群组分层限制工具的 `channels.*.groups.*.tools`，并明确 deny 覆盖 allow。在启用沙箱的运行时里，宿主机命令执行还额外受 `tools.elevated.*` 控制；elevated 表示允许工具从沙箱逃逸到宿主执行路径。如果沙箱本身关闭，普通 `exec` 已在宿主机上运行，`tools.elevated` 不会再提供额外隔离。工具策略的基础配置方法见 [5.2 工具策略：允许、拒绝与分层策略](../05_tools_skills/5.2_tool_policy.md)，本节侧重于安全防线视角下的联动配置。
...
`deny` 负责“这类工具总体能不能被调用”，而 `tools.elevated` 在沙箱已启用时进一步负责“是否允许该调用离开沙箱、落到宿主机执行域”。两层一起用，才能覆盖当前运行时真实存在的执行拦截链。
