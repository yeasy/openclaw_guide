...
**渠道审计的双模检查**：审计同时检查原始配置和运行时解析后的配置，能检测出凭据状态变化（如”配置中有 token 但运行时不可用”）。针对不同渠道有专项规则——如 Discord 的名称型 allowlist（可被改名绕过）、Telegram 的非数字用户 ID、Slack 的访问组旁路等。

**技能安全扫描器**：对已安装技能的 JS/TS 代码做静态分析，覆盖的典型风险包括：CRITICAL 级别的 `child_process` 调用、`eval()` 动态执行、环境变量外泄（`process.env` + 网络请求）、挖矿特征；WARN 级别的文件外泄组合、混淆代码、非标准端口 WebSocket 连接等。

**正则安全校验**：对用户提供的正则表达式做 ReDoS（正则拒绝服务）检测，识别嵌套重复模式（如 `(a+)+`），防止恶意技能通过构造灾难性回溯正则来耗尽 CPU。
