...
flowchart TD
  Start["选渠道"] --> Q1{"用户来自哪里？"}
  Q1 -->|"企业内部"| A["首选飞书<br/>/企业微信"]
  Q1 -->|"对外客服"| B["Telegram<br/>/WhatsApp"]
  Q1 -->|"开发者社区"| C["Discord<br/>（暂未支持）"]
...
openclaw channels login --channel whatsapp --account ops
```
...
2. **渠道检查**：用 `channels status --probe` 确认当前账号的链路、登录态与传输探针正常。
...
常见问题速查：
- **群聊不触发**：优先检查渠道侧 `groups.*.requireMention` 与智能体侧 `agents.list[].groupChat.mentionPatterns`；若是 Telegram，还要确认 BotFather Privacy Mode 是否满足你的触发方式。
