...
Telegram 渠道基于机器人令牌接入（对应 [Telegram Bot API](https://core.telegram.org/bots/api)，截至 2026-05-24，Bot API changelog 当前版本为 **10.0**；`sendMessageDraft` 最早在 9.3 加入，9.5 于 2026-03-01 放开所有 bot 使用，10.0 允许传空文本）。`dmPolicy` 支持 `pairing`、`allowlist`、`open`、`disabled` 四种选项，建议以白名单和提及门控作为安全起步。详见 OpenClaw 官方文档：[https://docs.openclaw.ai/channels/telegram](https://docs.openclaw.ai/channels/telegram)。
