    ...
    list: [
      ...
      },
      {
        ...
        bindings: [
          {
            channelId: “telegram”,
            groupId: “-1001234567890”,  // DevOps-team 群组 ID
            priority: 10                 // 优先级提高
          }
        ],
        toolGroups: [“exec”, “ssh”],     // 运维工具组
        tools: [“command_exec”, “ssh_connect”, “logs_query”]
      },
      {
        id: “writer”,
        displayName: “写作智能体”,
        ...
        bindings: [
          {
            channelId: “whatsapp”,
            peerId: “+86 15600000000”,    // 产品编辑对端号码
            priority: 10
          }
        ],
        toolGroups: [“document”],        // 文档编辑工具组
        tools: [“document_create”, “document_edit”, “document_publish”]
      }
    ]
  },
...
官方文档明确了绑定命中时的优先级顺序。当一条消息进入时，系统按以下顺序尝试匹配：

1. **Exact peer match**（精确对端匹配）：`peerId` 完全匹配，优先级最高。用于 DM 或已知固定对端的场景。
2. **Guild/team ID**（群组匹配）：`groupId` 或 `teamId` 匹配，适合在特定群内固定路由。
3. **Account ID (exact)**（账号精确匹配）：`accountId` 精确匹配，用于特定账号的全局绑定。
4. **Account ID (wildcard)**（账号通配）：`accountId` 使用通配符，支持多账号场景下的批量绑定。
5. **Default agent**（默认智能体）：当所有绑定都未命中时，路由器选择配置中的默认智能体（通常是 `assistant`）。

在上面的示例中，来自 DevOps-team 群的消息会优先命中 `devops` 的群组绑定，来自产品编辑号码的消息会优先命中 `writer` 的对端绑定，其他来源的消息默认路由到 `assistant`。
