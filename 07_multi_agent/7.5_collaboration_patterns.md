...
在 7.4 节中，我们通过绑定与路由把消息分发给正确的智能体，但单个智能体面对复杂任务时仍可能力不足。多智能体不仅用于入口路由，也用于把复杂任务拆成可并行的子任务，并把结果投递到多个目标群或多个对端。这种协作模式能够在保持边界清晰的同时，显著扩展系统的任务处理能力。本节基于 OpenClaw 的子智能体与广播组能力，给出两种常用协作模式的配置骨架与验收命令，并补充一套面向工程交付的交接协议，避免协作在”口头描述”上失真。
