...
1. ✅ **飞书侧**：创建应用 -> 配置权限 -> **发布应用（关键！）**
2. ✅ **OpenClaw 侧**：配置飞书渠道 (`openclaw channels add`)
...
**血的教训**：若在飞书侧还没有点击“创建版本并发布”（第 1 步），就直接跑到“事件与回调”里开启长连接（第 4 步），那么系统会无限报错“长连接订阅失败”。
...
### 7.3.3 在 OpenClaw 侧完成绑定

飞书并非内置渠道，因此需要在环境内先安装并启用相应插件，再配置渠道：

```bash
# 1. 下载并安装插件（必做，否则 enable 会提示找不到）
...
> [!NOTE]
> **`install` 与 `enable` 的区别及常见失败分支**：
> `install` 负责从 registry 下载代码包到本地环境；`enable` 负责将该插件注册到当前的 `openclaw.json` 配置中并进入激活态。新手常犯的错误是直接 enable。
> OpenClaw 插件分为 **bundled（内置）** 与 **非 bundled（独立）**。如果 `enable` 报错找不到要求，请强制执行 `install` 步骤先将插件引入本地 `extensions` 目录中。
...
5. 因为你在[第三章](../03_minimal_loop/README.md)配置了配对策略，此时会收到一个配对码（Pairing code）。复制这段码并在终端通过 `openclaw pairing approve feishu <CODE>` 批准。
6. 最后，享受你的企业专属私人助理！
