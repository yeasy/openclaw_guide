...
- 长连接订阅失败：优先确认已经先在 OpenClaw 里完成 `openclaw channels add`，并让 Gateway 处于运行态；否则飞书后台的长连接配置可能保存失败。
...
当前 OpenClaw 正式发行版已内置 Feishu 插件，因此正常安装后**不需要**先手动执行 `plugins install`。当前首选路径是使用渠道登录向导，它会优先尝试扫码创建/绑定配置，只有不可用时才回退到手动 App ID/App Secret：
