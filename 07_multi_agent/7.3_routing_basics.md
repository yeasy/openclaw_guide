...
openclaw 将“由谁接管”下沉为多智能体路由与绑定机制，并把“能做什么”交给工具策略与沙箱工具约束去兜底。路由层最重要的目标是把消息的处理权稳定地收敛到一个确定的智能体上，然后让后续执行在可控边界内进行。相关概念与配置细节见官方文档的多智能体路由与绑定说明：[https://docs.openclaw.ai/zh-CN/concepts/multi-agent-routing](https://docs.openclaw.ai/zh-CN/concepts/multi-agent-routing)，[https://docs.openclaw.ai/zh-CN/agents/multi-agent/bindings](https://docs.openclaw.ai/zh-CN/agents/multi-agent/bindings)。

### 7.3.2 决策链：先绑定后路由，绑定命中时直接接管

openclaw 的典型决策链可以概括为“先匹配绑定，再进入路由器”。绑定用于把某些来源的消息稳定地交给指定智能体处理，从而减少模型分类的不确定性。官方文档明确了绑定的优先级与用途，并给出了基于 `channelId`、`peerId`、`groupId` 等字段的匹配方式。绑定命中时，系统会直接将消息交给绑定的智能体处理。[https://docs.openclaw.ai/zh-CN/concepts/multi-agent-routing](https://docs.openclaw.ai/zh-CN/concepts/multi-agent-routing)
...
2. 任务层路由：入口智能体再将任务分发给领域智能体，或使用子智能体并行处理。子智能体机制见 [https://docs.openclaw.ai/zh-CN/agents/multi-agent/subagents](https://docs.openclaw.ai/zh-CN/agents/multi-agent/subagents)。
...
多智能体路由的关键是先把消息处理权收敛到确定智能体，再用工具策略与沙箱约束收敛“能做什么”。绑定机制把高确定性路由从概率问题变为规则问题；工程实践中应优先用绑定固定高风险入口，并用 `agents list --bindings`、`channels status --probe`、`status --deep`、`logs --follow --json` 把路由与执行边界做成可观测、可回放的链路。
