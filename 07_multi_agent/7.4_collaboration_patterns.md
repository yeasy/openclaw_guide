...
- 子智能体**不能再调用** `sessions_spawn`，因此最多只支持一层嵌套（无法派生下级子智能体）。
...
- 回告内容会被规范化为 `Status`、`Result`、`Notes` 这类结构。
- 如果子智能体没有最终文本回复，系统会回退到最近一次 `toolResult` 作为结果主体。

因此，写书时最好把“队列控制”和“子智能体回告”分开讲：前者解决入站并发，后者解决委派结果如何返还。
