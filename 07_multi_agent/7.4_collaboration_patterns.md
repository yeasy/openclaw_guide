...
在 7.3 节中，我们通过绑定与路由把消息分发给正确的智能体，但单个智能体面对复杂任务时仍可能力不足。多智能体不仅用于入口路由，也用于把复杂任务拆成可并行的子任务，并把结果投递到多个目标群或多个对端。这种协作模式能够在保持边界清晰的同时，显著扩展系统的任务处理能力。本节基于 OpenClaw 的子智能体与广播组能力，给出两种常用协作模式的配置骨架与验收命令，并补充一套面向工程交付的交接协议，避免协作在”口头描述”上失真。
...
主智能体可通过 slash command 手动派生，也可在自动化场景中通过 `sessions_spawn` 工具调用触发。基本 slash command 参考：
...
### 7.4.3 Announce Queue：子智能体结果投递的队列协议
...
参数覆盖优先级从高到低为：内联选项 > 会话级设置 > 按渠道防抖配置（`messages.queue.debounceMsByChannel`）> 渠道插件默认值 > 全局配置 > 硬编码默认值。
    ...
    list: [
      {
        id: "coordinator",
        displayName: "日报协调者",
        model: "gpt-4",  // 请根据实际可用模型名称替换
        tools: ["sessions_spawn", "summarize_reports", "broadcast_message"],
...
> [!NOTE]
> 在配置中提到的 `sessions_spawn`、`summarize_reports`、`broadcast_message` 等工具名称为示例，请根据实际的 OpenClaw 工具库与可用能力进行调整。建议通过 `/subagents spawn` slash command 进行子智能体派生，而非依赖工具调用。
...
```javascript
{
  mainAgentLogic: {
    ...
    step2: "使用 sessions_spawn 工具派生 code-reviewer：task = '分析最近24小时 Git 提交'",
    step3: "使用 sessions_spawn 工具派生 task-tracker：task = '统计今日完成的任务'",
...
# 查看定时任务的执行历史
openclaw cron runs --id daily_standup

# 手动触发一次日报生成（用于验证）
openclaw cron run daily_standup
...
如果出现“广播未送达”，优先检查目标群标识和渠道在线状态；如果子智能体未响应，检查 `agentId` 是否在 `agents.list` 中正确声明、工具策略是否允许 `sessions_spawn`。
