...
- `thread`、`mode`：在支持线程绑定的渠道里请求 thread-bound 交付；当前持久 thread-bound subagent session 的官方稳定支持应按 Discord 口径理解。
...
- 子智能体默认继承“全工具集减去 session 工具”，可用 `tools.subagents.tools.allow` / `alsoAllow` / `deny` 进一步调整；`allow` 是 allow-only，`alsoAllow` 用于恢复少量默认禁用项。
...
- `collect`：把排队消息合并成一次 followup，默认值。
- `steer-backlog`：先注入当前轮，再保留一条后续跟进。
- `steer+backlog`：`steer-backlog` 的等价写法（运行时规范化为同一模式）。
- `interrupt`：遗留模式，终止当前轮后处理最新消息。
- `queue`：遗留别名，等价于 `steer`。
