...
└── 其他工作区文件     # 可按需补充，但不属于默认注入集合
```

这些文件构成默认的工作区引导集合。当前官方文档明确会注入 `AGENTS.md`、`SOUL.md`、`TOOLS.md`、`IDENTITY.md`、`USER.md`、`HEARTBEAT.md`，以及**仅首轮运行**时生效的 `BOOTSTRAP.md`；其他文件即使存在，也不应默认理解为每次会话都会自动注入。各文件的详细职责、读取时机与加载策略将在后续章节展开：人格配置见[第 3 章](../03_minimal_loop/README.md)，心跳与定时任务见[第 8 章](../08_automation_ops/8.3_heartbeat.md)，上下文管理见[第 6 章](../06_context_memory/README.md)。
