...
向导结束后，如果使用了 `--install-daemon`，Gateway 通常已经由托管服务在后台运行；如果只运行了 `openclaw onboard`，则应先手动启动或确认本地 Gateway 进程。下一步是通过 Dashboard 完成首轮对话验证，详见[第三章](../03_minimal_loop/README.md)。
