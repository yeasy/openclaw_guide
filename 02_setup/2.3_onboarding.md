...
- **正常会话自动注入的 bootstrap 文件**：`AGENTS.md`、`SOUL.md`、`TOOLS.md`、`IDENTITY.md`、`USER.md`、`HEARTBEAT.md`，以及**仅在全新工作区首轮运行时**出现的 `BOOTSTRAP.md`；如果存在 `MEMORY.md`，它也会进入正常会话的 bootstrap 注入。
- **按场景触发或按需读取的工作区资产**：`BOOT.md` 只在启用对应内部 Hook 且 Gateway 启动时触发；`memory/` 中的日记式记忆文件通过 `memory_search` / `memory_get` 等工具按需读取；`skills/`、`canvas/` 则属于工作区附属目录，不会因为“文件存在”就自动进入每轮上下文。
