...
- **Podman**：无根容器方案，运行 `setup-podman.sh` 后即可启动。详见 [官方 Podman 安装指南](https://docs.openclaw.ai/install/podman)。
- **Nix**：声明式安装。详见 [官方 Nix 安装指南](https://docs.openclaw.ai/install/nix)。
- **Ansible**：自动化集群部署。详见 [官方 Ansible 安装指南](https://docs.openclaw.ai/install/ansible)。
- **From source**：克隆仓库后通过 `pnpm install && pnpm build` 编译。详见 [官方源码构建指南](https://docs.openclaw.ai/start/setup)。
