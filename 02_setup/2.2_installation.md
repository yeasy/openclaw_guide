...
如果验证失败，不要立即重复安装。先确认命令究竟安装到了哪个运行环境，以及该环境的全局 npm 目录是否在 PATH 中。OpenClaw 支持 Node 22.22.3+、24.15+ 或 25.9+（含 Node 26），新安装推荐 Node 26，Node 23 不受支持；版本和 PATH 规则以[官方 Node.js 安装说明](https://docs.openclaw.ai/install/node)为准。
...
# 会跳过 OpenClaw 的 preinstall/postinstall，需显式放行：
