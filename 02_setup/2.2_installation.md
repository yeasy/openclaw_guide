...
本节介绍如何在你选择的环境中安装 OpenClaw。我们为首次用户推荐**一键安装脚本**作为最快的成功路径——它会自动处理依赖并安装最新版 OpenClaw CLI。如果一键脚本不适用于你的环境，本节也列举了多种替代方案（npm、Docker、源码构建、Podman、Nix、Ansible）。
...
如果一键脚本不适用于你的环境，以下是其他安装方式。所有方式都可以成功安装 OpenClaw，选择最适合你的工具链和部署场景的方案。
...
#### 2.2.3.4 Podman、Nix、Ansible 安装

官方还支持以下安装方式，适用于特定运维场景：

- **Podman**：无根容器方案，通过 `./setup-podman.sh` 安装，支持 `--quadlet` 启用 systemd 集成。详见 [官方 Podman 安装指南](https://docs.openclaw.ai/install/podman)。
- **Nix**：声明式安装，推荐使用 [nix-openclaw](https://github.com/openclaw/nix-openclaw) Home Manager 模块，支持版本锁定与即时回滚。详见 [官方 Nix 安装指南](https://docs.openclaw.ai/install/nix)。
- **Ansible**：自动化集群部署，适合 Debian/Ubuntu 服务器，Playbook 会自动配置 VPN、防火墙、Docker、Node.js 及 systemd 服务。详见 [官方 Ansible 安装指南](https://docs.openclaw.ai/install/ansible)。
