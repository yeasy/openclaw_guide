...
如果你熟悉 Node 生态，或者需要在特定流程中进行精确版本控制，也可以直接使用 `npm` 或者 `pnpm` 进行全局安装。该路径仍要求 `git` 在 `PATH` 中可用：即使走 npm 安装，依赖里出现 git URL 时也会触发 `spawn git ENOENT` 一类错误；Windows 用户请先安装 Git for Windows 并重新打开 PowerShell。
