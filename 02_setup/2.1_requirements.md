...
1. **一台能上网的电脑**：支持 macOS、绝大多数 Linux 发行版或 Windows 的 WSL2 系统。只要是近 5 年的机器基本都能正常运行。最省事的办法是一台云虚拟机，但要注意能连通到互联网。
...
4. **内存与硬件规格**：最低 **4 GB** 内存，如果需要运行[浏览器工具](../05_tools_skills/5.4_browser_nodes.md)，推荐 **8 GB 或以上**。
...
1. **安装期网络**：确保机器能够正常拉取 npm registry 或 docker.io（如果使用容器化部署），必要时需配置 HTTP 代理及镜像源。
2. **运行时网络**：必须确保能够连通所选 LLM 供应商的接口。若要接入 Telegram Bot 等海外渠道平台，机器必须具备无阻碍地访问这些渠道的能力。
