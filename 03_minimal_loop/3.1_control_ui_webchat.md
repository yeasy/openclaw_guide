...
Dashboard（Control UI）是 OpenClaw 的 Web 管理中心。当前版本的左侧栏可以理解为 **四类主导航 + 1 个资源分区**：`Chat`、`Control`、`Agent`、`Settings`，以及独立的 `Resources` 区。下面先给出全景，再聚焦本章最常用的几个页面。
...
> **版本说明**：v2026.3 系列后的 Dashboard 已采用分组导航；具体栏目名称会继续演进，因此写书时应以当前 UI 文案为准，不要把分区数量写成不可变规则。
