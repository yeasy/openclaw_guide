...
`openclaw pairing list <channel>` / `openclaw pairing approve <channel> <code>` 是私聊渠道的 sender pairing 流程，不用于批准 Dashboard / WebChat 设备。
...
- 预期正常输出（流式返回）：
  ...
  ```
- 预期日志表现：
  - 能观察到流式增量输出或进度类事件；不要把 `chunk_sent` 当成跨版本固定事件名。
