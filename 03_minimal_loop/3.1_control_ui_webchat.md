...
Dashboard（Control UI）是 OpenClaw 的 Web 管理中心。根据当前本地界面与官方文档，左侧栏主要分为 **四组主导航**：`Chat`、`Control`、`Agent`、`Settings`。下面先给出全景，再聚焦本章最常用的几个页面。
...
- **Communications**：当前界面中的通信相关设置入口。
- **Appearance**：主题与显示风格等外观设置入口。
- **Automation**：自动化相关设置入口。
- **Infrastructure**：基础设施与环境设置入口。
- **AI & Agents**：AI 与智能体相关的全局设置入口。
...
- **Docs**：文档入口。当前界面中它归在 `Settings` 组下，不再作为独立的 `Resources` 分区出现。

> **版本说明**：v2026.3 系列后的 Dashboard 已采用分组导航；当前可见分组为 `Chat`、`Control`、`Agent`、`Settings`。具体栏目名称仍可能继续演进，因此写书时应以当前 UI 文案为准，不要把分组内页面数量写成不可变规则。
