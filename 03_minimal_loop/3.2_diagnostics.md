...
| 第三层：模型 | `openclaw models status --probe` | 模型认证、配额与供应商可用性 live probe |
...
flowchart TD
    ...
    S2 --> S3["openclaw models status --probe<br/>模型与配额"]
...
- `models status --probe` 失败 → 检查 API Key、配额与供应商状态；`--check` 仅用于检查自动状态是否缺失或过期。
