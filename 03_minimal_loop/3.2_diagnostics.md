...
| **第三层：模型** | `openclaw models status` / `--probe` | 认证状态与 live provider 探针 |
...
flowchart TD
    ...
    S2 --> S3["openclaw models status / --probe<br/>认证状态与live探针"]
...
- `models status` 显示认证或模型配置异常 → 检查 API Key、OAuth 过期或缺失；需要 live provider 证据时再运行 `models status --probe`。
