...
官方提供 `doctor` 用于诊断与修复常见问题，并提供诊断配置用于脱敏与控制日志落盘。参考：https://docs.openclaw.ai/cli/doctor 与 https://docs.openclaw.ai/zh-CN/gateway/configuration#diagnostics。
...
> [!WARNING]
> 飞书渠道目前不支持斜杠命令系统。如果你的主要接入渠道是飞书，建议优先使用 SSH 终端运行 `openclaw` CLI 命令来完成等效操作，或考虑切换到 Telegram 等支持斜杠命令的渠道。
