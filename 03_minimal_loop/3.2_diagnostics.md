...
| 步骤 | 命令 | 检查目标 |
|------|------|----------|
| 1 | `openclaw health --json` | 进程与网关是否存活 |
...
正常返回 `"status": "ok"`；出现 `"degraded"` 时优先查看 `errors` 数组，常见原因是渠道 token 过期或模型配额告警。
...
> **踩坑实录：健康检查 "ok" 但消息不通**
>
> `openclaw health --json` 返回 `status: "ok"`，但 WhatsApp 消息始终无回复。原因是 Baileys WebSocket 连接断开，进程仍在跑。`channels status --probe` 才是真正的端到端探测命令，`health` 只检查进程存活。生产环境建议两者都纳入监控。
