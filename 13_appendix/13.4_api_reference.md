...
本附录提供“快速定位入口”的参考，不替代 API 文档。字段与接口会随版本变化：如果你发现字段或命令不同，优先用 CLI 的 `--help` 与 `status --deep`/结构化日志自证，再回到 D.2 的最小排障集核对结果。
...
如果你需要在应用中嵌入 OpenClaw 能力，建议采用“先 CLI 验证、再 SDK/HTTP 集成”的顺序：
