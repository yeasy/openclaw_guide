...
- 工具结果裁剪：https://docs.openclaw.ai/zh-CN/gateway/configuration#agentsdefaultscontextpruning
...
`agents.defaults.contextPruning` 的关键调参项包括 `keepLastAssistants`、软硬阈值比率、以及 `tools.deny` 排除列表。参考：[工具结果裁剪](https://docs.openclaw.ai/zh-CN/gateway/configuration#agentsdefaultscontextpruning)。
...
官方提供 `agents.defaults.compaction`，并支持在压缩前触发 `memoryFlush`，提示模型把长期记忆写入到工作区文件（例如每日记忆文件）。参考：[压缩配置](https://docs.openclaw.ai/zh-CN/gateway/configuration#agentsdefaultscompaction)。
...
注意：当工作区为只读时会跳过记忆刷新。相关限制见官方说明：https://docs.openclaw.ai/zh-CN/gateway/configuration#agentsdefaultscompaction。
...
- 工具结果裁剪：https://docs.openclaw.ai/zh-CN/gateway/configuration#agentsdefaultscontextpruning
- 压缩与记忆刷新：https://docs.openclaw.ai/zh-CN/gateway/configuration#agentsdefaultscompaction
