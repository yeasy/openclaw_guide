...
本节将”上下文爆炸”拆分为两类可配置机制：工具结果裁剪（Pruning）与会话压缩（Compaction）。前者由 `agents.defaults.contextPruning` 控制，目标是裁掉旧工具结果以降低模型输入体积；后者由 `agents.defaults.compaction` 控制，目标是在会话接近压缩阈值时生成摘要，并在必要时先刷新长期记忆。两者配合，才能在长会话中同时保证可用性与可回放性。
