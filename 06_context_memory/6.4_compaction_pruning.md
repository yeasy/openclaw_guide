...
**Token 估算**：源码使用保守的 `CHARS_PER_TOKEN_ESTIMATE = 4`（英文约 4 字符 ≈ 1 token；**中文文本每个 Token 约 1.5-2 个字符，混合文本按实际运行时统计，实际消耗可能显著高于英文估算值**），图片按 `IMAGE_CHAR_ESTIMATE = 8000` 字符计入预算。这意味着一张图片约占 2000 token 的预算。
