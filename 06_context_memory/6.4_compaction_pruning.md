...
了解裁剪的主要参数有助于在生产环境中精细调优——比如调整保护多少条最近的助手回复、在什么体积阈值触发裁剪等。`agents.defaults.contextPruning` 的主要默认参数：
...
| `tools` | `{}` | 可按工具覆盖裁剪行为 |
| `hardClear.enabled` | `true` | 是否启用硬清除 |
| `hardClear.placeholder` | `"[Old tool result content cleared]"` | 硬清除后回填的占位文本 |
