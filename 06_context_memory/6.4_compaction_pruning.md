...
- 上下文裁剪（Session Pruning）：发生在“发送给模型之前”，只影响本轮输入，不改磁盘历史。
- 会话压缩（Compaction）：发生在“会话接近上限时”，通过摘要把历史折叠为可继续推理的状态块，并持久化到 JSONL。
...
> [!IMPORTANT]
> Session Pruning 目前**仅在 `mode: "cache-ttl"` 且使用 Anthropic API 时生效**（含 OpenRouter 上的 Anthropic 模型）。Compaction 是独立机制，两者不互相依赖。
...
官方提供 `agents.defaults.compaction`，并支持在压缩前触发 `memoryFlush`，提示模型把长期记忆写入到工作区文件（例如每日记忆文件 `memory/YYYY-MM-DD.md`）。参考：[压缩配置](https://docs.openclaw.ai/gateway/configuration#agentsdefaultscompaction)。

配置示例（以下数值为示意，官方示例以 `reserveTokensFloor: 20000`、`softThresholdTokens: 4000` 为参考基准，请以官方文档为准）：
...
注意：当工作区为只读（`workspaceAccess: "ro"` 或 `"none"`）时会跳过记忆刷新。相关限制见官方说明：https://docs.openclaw.ai/gateway/configuration#agentsdefaultscompaction。
