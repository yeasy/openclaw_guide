...
消息在会话中的排队方式会影响实际体验。OpenClaw 官方队列模式包含 `collect`（默认）、`steer`、`followup`、`steer-backlog` 和 `interrupt`。核心区分在于：`collect` 按顺序逐条处理，适合独立长任务；`steer` 实时注入新消息使智能体立刻调整方向，适合人机协作纠偏场景。
