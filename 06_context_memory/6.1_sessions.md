...
当 OpenClaw 面向多个不受信用户开放 DM 私聊时，需要理解隔离的两层含义：会话层面（Token 窗口、历史记录、上下文）**并不是默认按用户隔离**；当前默认更偏向“所有 DM 共享主会话以保持连续性”，只有在显式配置 `session.dmScope` 或 `session.identityLinks` 等策略后，才会按用户、渠道或跨渠道映射进行更细粒度拆分。与此同时，宿主机资源（文件系统、Shell）仍是共享的，若工具策略未做沙箱限制，用户依然可能间接访问全局信息。
