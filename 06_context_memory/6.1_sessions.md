...
消息在会话中的排队方式会影响实际体验。OpenClaw 配置中可接受的队列模式值有 `collect`、`steer`、`followup`、`steer-backlog`、`steer+backlog`、`steer_backlog`、`queue`、`queued` 和 `interrupt`。其中 `steer+backlog` 与 `steer_backlog` 会归一化为 `steer-backlog`，`queue` 与 `queued` 会归一化为独立的 `queue` 模式，而不是 `steer` 的别名；因此独立行为共 6 种。核心区分在于：`collect` 合并消息后再处理；`steer` 实时注入新消息使智能体调整方向；`queue` 是 one-at-a-time steering 模式，适合把纠偏消息逐条串行化。
