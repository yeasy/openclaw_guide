...
本节以官方配置为准讲清会话的三件事：会话键如何划分作用域；会话如何重置与归档；会话存储写到哪里、如何验证与排障。目标是把“串话、重复执行、恢复失败”这三类问题，从模型层拉回到可配置、可观测、可回放的工程层。

### 6.1.1 会话作用域：先把会话键划清楚

OpenClaw 的会话行为由全局 `session` 配置控制。最重要的旋钮是 `session.scope`，用于定义“哪些消息会被折叠到同一条会话里”。官方示例与字段解释见：[会话配置](https://docs.openclaw.ai/zh-CN/gateway/configuration#session)。
...
长时间运行后，会话会积累历史与上下文偏差。官方提供了按时间或按空闲时间重置的配置，并支持按会话类型分别设置（例如私聊更长，群聊更短）。参考：[会话配置](https://docs.openclaw.ai/zh-CN/gateway/configuration#session)。
...
> [!TIP]
> 如果你经常需要在智能体执行过程中修改方向（例如"先别搜了，换个关键词"），建议开启 steer 模式。如果你的智能体主要处理独立任务且不希望被打断，保持默认的 queue 模式即可。

### 6.1.4 会话存储：默认位置与覆盖方式

会话默认按智能体存储在 `~/.openclaw/agents/<agentId>/sessions/sessions.json`。官方允许用 `session.store` 覆盖写入位置，并支持 `{agentId}` 模板。参考：[会话配置](https://docs.openclaw.ai/zh-CN/gateway/configuration#session)。
...
- 会话机制：https://docs.openclaw.ai/zh-CN/agents/sessions
- 配置参考：https://docs.openclaw.ai/zh-CN/gateway/configuration#session
