...
在 OpenClaw 中，工作区是上下文的第一来源。官方记忆机制明确了几个关键文件的用途：系统提示、技能、工作区说明、长期记忆与每日日志。参考：[记忆](https://docs.openclaw.ai/zh-CN/agents/memory)。

上下文的工程挑战并不在“有没有信息”，而在“信息是否以可用形态被注入”。典型反模式是把大量工具原始输出长期堆在会话里，导致成本、时延与注意力稀释一起失控。

### 6.2.2 工具结果裁剪：agents.defaults.contextPruning

官方提供 `agents.defaults.contextPruning`，用于在请求发送到模型之前裁剪旧工具结果。关键点是：它只改变“发送给模型的上下文”，不会修改磁盘上的会话历史，便于回放与审计。参考：[工具结果裁剪](https://docs.openclaw.ai/zh-CN/gateway/configuration#agentsdefaultscontextpruning)。

两种模式。

- 自适应：先按比例软裁剪过大的工具结果，再在达到硬阈值时清除最旧的工具结果。
- 激进：直接用占位符替换更旧的工具结果。

配置示例（官方默认是自适应，这里补充常用调参项）：
