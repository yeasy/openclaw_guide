...
本节以 OpenClaw 的实际机制为准讨论上下文预算：上下文由工作区、技能、会话历史与工具回执共同组成；当工具输出持续累积时，需要在”发送给模型之前”做有规则的裁剪（Pruning），而不是修改磁盘历史。重点落在 `agents.defaults.contextPruning` 的行为与调参方法，以及如何用回放与指标验证裁剪没有破坏关键决策。
...
> [!IMPORTANT]
> 上下文裁剪（Pruning）目前 **仅在 `mode: “cache-ttl”` 且使用 Anthropic API 时生效**（含 OpenRouter 上的 Anthropic 模型）。其核心目的是在会话空闲超过 prompt cache TTL 后，裁剪旧工具结果以减少重新缓存的成本。若使用 OpenAI 或其他供应商，此功能暂不适用。
...
当出现”工具已调用但模型忽略”时，优先检查工具回注是否被硬清除，而不是直接修改提示词。
...
> **📚 延伸阅读**：上下文工程是智能体性能的关键杠杆。关于上下文窗口管理、信息密度优化和多轮对话架构的系统性方法论，参见 [《大模型上下文工程权威指南》](https://github.com/yeasy/context_engineering_guide)。
