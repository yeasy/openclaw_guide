...
> [!TIP]
> 不确定选 Heartbeat 还是 Cron？快速判断：需要精确时间 → Cron；需要周期性感知并按需通知 → Heartbeat。详细对比见本节末尾。
...
下面从源码实现角度还原 Heartbeat 的完整执行流程。这是理解其行为的关键，也有助于排障。
...
│                                                                 │
│  ① 定时器调度                                                    │
│  startHeartbeatRunner() 为每个智能体维护独立的                     │
│  HeartbeatAgentState，计算 nextDueMs = lastRunMs + intervalMs    │
│  ↓ setTimeout() 到期                                            │
│  ↓                                                              │
│  ② 唤醒与合并                                                    │
│  requestHeartbeatNow() 将唤醒原因入队                             │
│  多个触发源（interval/cron/exec/manual）在 250ms 窗口内合并       │
│  优先级：RETRY(0) < INTERVAL(1) < DEFAULT(2) < ACTION(3)        │
│  ↓                                                              │
│  ③ 预检门控                                                      │
│  · 智能体是否启用心跳？                                           │
│  · 是否在活跃时段内？（activeHours）                               │
│  · 主队列是否空闲？（有请求在飞则跳过并 retry）                    │
│  · HEARTBEAT.md 是否存在且非空？（空文件 → 跳过以节省 API 费用）   │
│  ↓ 全部通过                                                      │
│  ④ 提示词装配                                                    │
│  · 分类触发原因（exec-event / cron / wake / interval）            │
│  · 构建对应提示词（标准 / buildCronEventPrompt / buildExecEvent…） │
│  · appendCronStyleCurrentTimeLine() 附加当前时间                   │
│  · 附加 HEARTBEAT.md 路径提示                                     │
│  ↓                                                              │
│  ⑤ 调用 LLM                                                     │
│  getReplyFromConfig(ctx, { isHeartbeat: true }, cfg)             │
│  可选：model 覆盖、lightContext 轻量上下文模式                     │
│  ↓                                                              │
│  ⑥ 回复标准化                                                    │
│  · 剥离 HTML/Markdown 标记（如 <b>HEARTBEAT_OK</b>）              │
│  · stripHeartbeatToken() 检测首尾的 HEARTBEAT_OK                  │
│  · 残余文本 ≤ ackMaxChars（默认 300）→ shouldSkip = true          │
│  ↓                                                              │
│  ⑦ 跳过 / 投递决策                                               │
│  · 空回复 → 发送 HEARTBEAT_OK（如果 showOk=true），事件 ok-empty  │
│  · 仅含 token → 回滚 session updatedAt，事件 ok-token            │
│  · 与上次相同文本 → 去重跳过，事件 skipped:duplicate              │
│  · 有实质内容 → 进入投递流程                                      │
│  ↓                                                              │
│  ⑧ 投递目标解析                                                  │
│  resolveHeartbeatDeliveryTarget():                               │
│  · target: "none" → 只运行不投递                                  │
│  · target: "last" → 查 session 上次联系的渠道                     │
│  · target: "<channel>" → 指定渠道 + 可选 to / accountId           │
│  · directPolicy: "block" → 屏蔽私聊投递                           │
│  ↓                                                              │
│  ⑨ 可见性解析                                                    │
│  resolveHeartbeatVisibility():                                   │
│  优先级：per-account > per-channel > channel defaults > 全局默认  │
│  · showOk=false, showAlerts=true, useIndicator=true（默认）       │
│  · 三者全 false → 直接跳过整个心跳轮次                             │
│  ↓                                                              │
│  ⑩ 渠道就绪检查 & 发送                                           │
│  plugin.heartbeat.checkReady() → deliverOutboundPayloads()       │
│  可选附带 reasoning payloads（includeReasoning=true）             │
│  ↓                                                              │
│  ⑪ 事件发射与清理                                                │
│  emitHeartbeatEvent({ status, preview, durationMs, ... })        │
│  裁剪 transcript 中的 HEARTBEAT_OK 轮次                           │
│  推进 advanceAgentSchedule() → scheduleNext()                    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
**定时器是 per-agent 的。** `HeartbeatAgentState` 为每个智能体独立维护 `intervalMs`、`lastRunMs`、`nextDueMs`，这意味着多智能体场景下各自按自己的节奏心跳，不会相互阻塞。当配置热更新时，`updateConfig()` 会重新计算所有智能体的间隔。

**唤醒合并防止风暴。** 多个触发源（定时器到期、Cron 事件、exec 完成）可能在短时间内同时请求心跳。系统用 250ms 的合并窗口将它们归并为一次执行，且按优先级保留最高等级的唤醒原因。

**心跳不续命。** 如果心跳回复是 `HEARTBEAT_OK`（无实质内容），网关会**回滚** session 的 `updatedAt` 时间戳到心跳前的值。这是为了确保空闲超时（idle expiry）正常工作——纯心跳不应该无限延长会话寿命。

### 8.3.3 发给 LLM 的完整消息结构

理解心跳的关键在于搞清楚每次心跳轮次**到底给 LLM 发了什么**。它由三部分组成：系统提示词、项目上下文和用户消息。

#### 系统提示词中的 Heartbeat 段

在系统提示词构建阶段（`system-prompt.ts`），当 `promptMode` 不是 `"minimal"` 时，网关会注入一个专门的 Heartbeat 段落：
...
这段文本告诉 LLM 心跳的响应协议——如何回复 `HEARTBEAT_OK` 以及何时不回复。注意，如果使用轻量上下文模式（`lightContext: true`），这个段落**不会**被注入。

#### 项目上下文（Bootstrap Files）
...
源码中的过滤逻辑（`bootstrap-files.ts`）：

```typescript
if (contextMode === "lightweight" && runKind === "heartbeat") {
...
默认提示词可通过 `heartbeat.prompt` 完全替换（注意是替换而非合并）。尾部的时间行由 `appendCronStyleCurrentTimeLine()` 自动追加，格式为 `Current time: [本地时间] ([时区]) / [UTC时间] UTC`。

**② Cron 事件触发**

当 Cron 作业（`--session main --wake now`）产生系统事件时，下一次心跳会自动拾取：
...
| 工具错误告警 | `suppressToolErrorWarnings: true` 时抑制工具错误告警 |
| 打字指示器 | 影响渠道端的 typing 通知行为 |
| 转录裁剪 | 回复 HEARTBEAT_OK 后裁剪 transcript，避免上下文污染 |

### 8.3.4 HEARTBEAT_OK 响应协议
...
源码中的标准化过程还会处理 LLM 偶尔添加的格式标记——`<b>HEARTBEAT_OK</b>` 和 `**HEARTBEAT_OK**` 都会被正确识别和剥离。
...
`activeHours` 通过 `isWithinActiveHours()` 在心跳触发前做时区感知的过滤：

1. 将 `start` 和 `end` 解析为分钟数（0–1440）
2. 通过 `Intl.DateTimeFormat()` 获取配置时区下的当前时间
3. 判断当前时间是否落在窗口内（支持跨午夜的窗口）
4. 不在窗口内的心跳被跳过，日志记录原因 `quiet-hours`
...
每次心跳执行都会发射 `HeartbeatEventPayload` 事件，供 UI 和监控消费：
...
系统事件不仅限于手动——Cron 作业和异步命令（exec）完成时也会产生事件，心跳会在下一次触发时自动纳入处理。源码中的提示词构建逻辑会检查待处理的事件队列，针对不同事件类型生成专属提示词（如 Cron 提醒内容会被直接嵌入提示词体内）。

### 8.3.11 Heartbeat vs Cron：选型决策树
