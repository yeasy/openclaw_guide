...
如果多个智能体配置了心跳，`--mode now` 会立即唤醒所有已配置 heartbeat 的智能体；`--mode next-heartbeat` 则等到下一次计划 tick 再处理。

系统事件不仅限于手动——Cron 作业和异步命令（exec）完成时也会产生事件，心跳会在下一次触发时自动纳入处理。系统会检查待处理的事件队列，针对不同事件类型生成专属提示词（如 Cron 提醒内容会被直接嵌入提示词体内，见 [8.3.3 HEARTBEAT_OK 协议与上下文模式](#833-heartbeatok-协议与上下文模式)）。
