...
当 `promptMode` 不是 `"minimal"` 时，网关在系统提示词中注入心跳协议说明。`lightContext: true` 会限制心跳上下文和 bootstrap 内容，但不会自动移除这段系统提示；需要省略心跳协议说明时，应设置 `heartbeat.includeSystemPromptSection: false` 或关闭心跳 cadence。
