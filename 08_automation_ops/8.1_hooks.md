...
### 8.1.2 OpenClaw 的 Hook 机制与配置入口

OpenClaw 通过 **hooks 目录** 提供 Hook 能力。每个 Hook 是一个独立目录，包含描述文件（`HOOK.md`）和处理逻辑（`handler.ts` 或 `handler.js`），由 Gateway 在指定事件点自动加载执行。
...
```bash
openclaw hooks list               # 查看所有已注册的 Hook 及其状态
openclaw hooks info <hookName>    # 查看指定 Hook 的详情
openclaw hooks enable <hookName>  # 启用 Hook
openclaw hooks disable <hookName> # 禁用 Hook
```
...
> [!NOTE]
> Hook 的注册方式与事件列表可能随版本演进。以 `openclaw hooks --help` 与结构化日志的实际输出为自证入口。
