...
在当前版本中，Hook 体系既包含 Gateway 内建 hooks，也支持从目录和插件中发现 hooks，并可用 `openclaw hooks` 系列命令检查。目录式 Hook 不应被当成每个用户都必须手写的起点，但它是当前官方支持的发现机制之一。常见配置入口包括：

- `hooks.internal.enabled`
- `hooks.internal.entries.session-memory.enabled`
- `hooks.internal.entries.boot-md.enabled`

也就是说，当前版本更稳妥的理解方式是：**Hook 是 Gateway 生命周期中的可发现切入点**。来源可能包括 bundled hooks、plugin hooks、`~/.openclaw/hooks/`、`hooks.internal.load.extraDirs`，以及显式启用的 workspace hooks。是否实际加载，取决于 `hooks.internal.enabled`、具体 entry、hook pack、legacy handler 或额外目录配置。
...
> [!NOTE]
> 自证入口优先使用 `openclaw hooks list`、`openclaw hooks info <name>`、`openclaw hooks check`，再结合 `config.get`、`config.schema`、`doctor` 与结构化日志。workspace hooks 默认不会覆盖 bundled、managed 或 plugin 中的同名 hook，启用前应先确认来源与优先级。
