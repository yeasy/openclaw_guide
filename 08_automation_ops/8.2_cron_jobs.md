...
如果你拿不准该选 Cron 还是 Heartbeat，这里只做**首轮分流**：**优先问”是否要求精确时间点”**。如果是，用 Cron；如果不是（比如”只要每半小时检查一遍是否有新工单，有就通知”），用 Heartbeat。更细的会话、上下文和模型差异，统一放到 [8.3.8 Heartbeat vs Cron](8.3_heartbeat.md#838-heartbeat-vs-cron) 再展开，避免同一问题在两节里重复讲两遍。
