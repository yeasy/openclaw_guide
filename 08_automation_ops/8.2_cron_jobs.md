...
- **隔离任务**：使用 `--message`，通常配合 `--session isolated`，在专用 `cron:<jobId>` 会话里运行；是否回退投递给用户取决于 `announce` / `webhook` / `none` 等 delivery 配置，不能简单理解为“isolated 默认通知”。
...
> [!NOTE]
> 以上命令参数以实际 CLI 版本为准。使用 `openclaw cron --help` 确认当前可用的参数格式。当前版本对**不带时区**的 `--at` 时间按 UTC 解释；若要按本地墙上时间理解，应显式传入 `--tz <IANA 时区>`。周期任务在整点附近可能自动错峰；如果文案承诺“9:00 准时”这类精确墙上时间，应显式加 `--exact`。
...
- **隔离会话模式**（`--message` / `--session isolated`）：作业在独立的 `cron:<jobId>` 会话中执行，空白上下文起步，适合不需要历史对话的独立任务；runner 是否回退投递结果，要看 `announce` / `webhook` / `none` 配置，同时 agent 也可能在作业内部主动使用 `message` 工具发送消息。
