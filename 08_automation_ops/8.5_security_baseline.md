...
下面示例展示了常见的密钥注入形态：可使用简单的插值，或是依赖 `SecretRef` 将它对接统一凭据管线。实际部署时建议为不同环境配置不同的 auth profile、环境变量或 SecretRef 来源。

```jsonc
{
  env: {
    // 方式一：由进程环境或 ~/.openclaw/.env 提供
  ...
  auth: {
    profiles: {
      ...
      "openai:staging": {
        provider: "openai",
        mode: "api_key",
        keyRef: { source: "env", provider: "default", id: "OPENAI_API_KEY_STAGING" },
      },
    },
    order: {
      openai: ["openai:prod", "openai:staging"],
    },
  }
}
```
