...
```
graph TD
    subgraph Layer1 [1. 入口层：协议适配与事件接入]
        Channels["多渠道收件箱<br>(WhatsApp / Telegram / Slack 等)"]
    end

    subgraph Layer2 [2. 控制层：全局中枢与安全门控]
        Gateway["Gateway<br>(身份鉴权 / 终端配对 / 多智能体路由)"]
    end

    subgraph Layer3 [3. 执行层：推理引擎与会话驱动]
        Agent["Pi Agent 引擎<br>(提示词组装 / 工具调度)"]
        Session["Session<br>(历史语境提取)"]
    end

    subgraph Layer4 [4. 能力层：可替换的基础底座]
        Models["大模型供应商<br>(意图推理)"]
        Tools["生态工具集<br>(浏览器 / 节点设备 / 实时语音)"]
    end

    Channels -->|统一内部事件| Gateway
    Gateway -->|受控准入请求| Agent
    Agent <-.->|读写状态| Session
    Agent -->|请求推理| Models
    Agent -->|调用执行| Tools
```
