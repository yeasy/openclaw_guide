...
flowchart LR
  subgraph Inbound["入口与渠道"]
    U["用户消息"] --> CH["Channels\n(WhatsApp/Telegram/Feishu/...)"]
  end

  subgraph Gateway["Gateway 控制平面"]
    CH --> GW["Gateway\nWebSocket + HTTP + Control UI"]
    GW --> ROUTE["Routing/Bindings\nSessionKey 决策"]
    ROUTE --> RUN["Agent Runtime\nPrompt 装配/状态机"]
    RUN --> MODEL["Model Providers\n(按策略选择/回退)"]
    RUN --> TOOL["Tools\n(受策略/沙箱/审批约束)"]
    RUN --> OUT["Reply Stream"]
    RUN --> STORE["Session Store + Transcript\nsessions.json + *.jsonl"]
    GW --> LOGS["File Logs (JSONL)\nControl UI/CLI tail"]
  end
