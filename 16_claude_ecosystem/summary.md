...
- **Agent SDK 与 OpenClaw 互补**：Agent SDK 侧重编排逻辑开发，OpenClaw 侧重运行平台管控（渠道接入、安全审批、会话管理），生产系统中可通过 MCP、已安装启用的 ACP/acpx backend 或受信插件接入 Agent SDK 应用；默认 Codex 路径仍是原生 Codex app-server `/codex` 与 `openai/*` runtime。
