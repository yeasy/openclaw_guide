...
- **MCP 是工具扩展的标准路径**：通过 stdio 或 Streamable HTTP 接入外部 MCP 服务器；SSE 主要用于旧远程 MCP 兼容。MCP 工具与 OpenClaw 内置工具并行存在，由模型推理阶段统一调度。
- **多智能体协作有两种架构路径**：基于 `agentId`、`accountId`、`bindings` 的确定性路由适合业务域分工，基于 `sessions_spawn` 的任务委托适合专家协作；两者可组合使用。
- **Agent SDK 与 OpenClaw 互补**：Agent SDK 侧重编排逻辑开发，OpenClaw 侧重运行平台管控（渠道接入、安全审批、会话管理），生产系统中可通过 MCP、已安装启用的 ACP/acpx backend 或受信插件接入 Agent SDK 应用；默认 Codex 路径仍是原生 Codex app-server `/codex`，模型标识按当前认证路径区分 `openai/*` 与 `openai-codex/*`。
- **多生态混合是生产常态**：OpenAI 通过原生 OpenAI/Codex/Azure 路由接入，OpenAI-compatible 仅用于通用代理或本地兼容端点；Ollama 本地模型适合隐私优先或离线兜底场景；不同生态可按任务复杂度和成本要求灵活组合。
