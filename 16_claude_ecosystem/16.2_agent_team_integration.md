...
实现方式：协调 Agent 通过 `sessions_spawn` 或 `/subagents spawn` 发起后台子任务；`/subagents spawn` 立即返回 run id，`sessions_spawn` 返回 `runId` / `childSessionKey`，完成后由 OpenClaw push / announce 结果。
