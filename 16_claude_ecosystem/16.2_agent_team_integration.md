    ...
    list: [
      {
        ...
        model: { primary: "anthropic/<当前轻量 Claude 模型>" },
      },
      {
        id: "ops",
        name: "Ops",
        ...
        model: { primary: "anthropic/<当前 Sonnet 主力模型>" },
      },
      {
        id: "analyst",
        name: "Analyst",
        ...
        model: { primary: "anthropic/<当前旗舰 Claude 模型>" },
      },
    ],
  },
  bindings: [
...
实现方式：入口 Agent 使用轻量模型（如当前可用的 Haiku / 轻量 Sonnet）快速分类请求意图，然后将任务转交给对应的专家 Agent。这种模式的好处是路由策略可以随对话上下文动态调整，而不受静态入口绑定的限制。
