...
实现方式：协调 Agent 通过 `sessions_spawn` 或 `/subagents spawn` 发起后台子任务；`/subagents spawn` 立即返回 run id，`sessions_spawn` 返回 `runId` / `childSessionKey`，完成后由 OpenClaw push / announce 结果。只有在 ACP/acpx backend 已安装、启用并可用时，才应使用 `sessions_spawn({ runtime: "acp" })`；默认 Codex 路径应走原生 Codex app-server `/codex` 与 `openai/*` runtime。
