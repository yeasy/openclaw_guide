...
OpenClaw 通过不同 provider path 与 runtime 接入 OpenAI 相关生态。直接 OpenAI API Key 路径使用 `openai/*`；ChatGPT/Codex 订阅 OAuth 路径当前使用 `openai-codex/*` 模型引用，并以 `openai-codex` auth profile 表达凭据。不要把历史兼容的 `codex/*`、OpenAI Platform API 路径和 Codex 订阅路径混写。当前模型版本、别名和价格变化很快；截至 2026-05-21，长期配置应以 `openclaw models list`、`models status --probe`、OpenAI 官方 Models/Pricing 页和目标 OpenClaw 版本为准。
