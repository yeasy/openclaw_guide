...
Anthropic 的 Claude 模型长期按能力与成本分为三个系列（2026-06-09 起，新一代 Fable 5 / Mythos 5 位于这三档之上）：
...
| **Opus 4.8** | Anthropic 最强 Opus 档推理与长程编码模型 | Anthropic 官方默认 1M；OpenClaw 可用性以当前 provider/model catalog 为准 | 默认 `effort=high`，thinking 细节以当前 provider/model catalog 为准 |
| **Fable 5** | 新一代旗舰：能力最强的广泛发布模型（2026-06-09 GA，$10/$50） | Anthropic 官方 1M、128K 输出；OpenClaw 可用性以当前 provider/model catalog 为准 | Adaptive Thinking 常开（不可关闭）；同日发布的 Mythos 5 仅限受邀 |
...
Fallback 链路的配置方式和行为详见 [4.4 故障转移基础](../04_config_models/4.4_failover.md)。
