...
```
┌──────────────────────────────────┐
│ 1. 提示词装配 (Prompt Assembly)  │  初始输入
│    ├─ 系统提示 (System Prompt)   │  ~500-1000 tokens
│    ├─ 工具定义                   │  ~200-500 tokens
│    ├─ 用户消息                   │  ~50-1000 tokens
│    └─ 上下文历史                 │  ~500-5000 tokens
└──────────────────────────────────┘
                │
                ▼
┌──────────────────────────────────┐
│ 2. 模型推理 (LLM Inference)      │  关键成本点
│    └─ 输入Token + 输出Token      │  按模型定价
└──────────────────────────────────┘
                │
                ▼
┌──────────────────────────────────┐
│ 3. 工具调用解析                  │  额外消耗
│    └─ 工具参数提取               │  ~50-200 tokens
└──────────────────────────────────┘
                │
                ▼
┌──────────────────────────────────┐
│ 4. 结果注入与继续推理             │  循环消耗
│    ├─ 工具返回值                 │  ~100-2000 tokens
│    └─ 继续推理                   │  ~50-500 tokens
└──────────────────────────────────┘
```
