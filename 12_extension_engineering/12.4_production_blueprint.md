...
官方插件体系支持通过 `enabled` 显式启停插件，并通过 `plugins.allow` 与 `plugins.deny` 收敛可加载范围：[https://docs.openclaw.ai/tools/plugin](https://docs.openclaw.ai/tools/plugin)。需要特别注意：**插件配置变更需要 Gateway 重启后才会生效**；默认 `openclaw gateway` 路径通常会自动完成一次 watcher 驱动的重启，但容器、远端主机或托管环境仍应显式确认 live gateway 已完成重载。
