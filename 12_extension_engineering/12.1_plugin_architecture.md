...
此外，OpenClaw 可以发现并安装兼容的 **Codex、Claude 和 Cursor Bundle**。系统会自动将 Bundle 中的技能映射为 OpenClaw 技能，将 `settings.json` 中的默认配置应用到 embedded Pi 运行环境，同时对 shell 相关覆盖项进行安全清洗。这意味着社区中大量为其他 AI 编码工具编写的 Bundle 可以直接在 OpenClaw 中复用。
