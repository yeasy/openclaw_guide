...
1. **阶段一：白名单准入** ——只在 `plugins.allow` 中添加该插件，并在目标入口的默认群组策略里拒绝其写入工具，先只允许查询功能。不要把这些写工具放进全局 `tools.deny`，否则阶段二无法再按发送者放开：
...
```jsonc
{
  ...
  channels: {
    telegram: {
      groups: {
        '*': {
          tools: { deny: ['jira.create*', 'jira.update*', 'jira.delete*'] },
        },
      },
    },
  },
...
3. **阶段三：全量放开** ——在内部验证 2 周无异常后，移除入口默认策略中的 Jira 写入限制，但保留审计日志与幂等键。
...
**结果合并策略**因钩子类型而异：`before_model_resolve` 采用首个非空结果获胜；`before_prompt_build` 拼接上下文与系统提示词；`before_tool_call` 只会在未被终止前继续合并参数，一旦某个 handler 返回 `{ block: true }` 就会终止后续链路；`message_sending` 首个取消操作获胜。
...
1. **发现**：从 bundled、global、workspace、config 等来源扫描候选插件与兼容 bundle。
2. **校验**：对原生插件读取 `openclaw.plugin.json`，并校验相关 schema；对兼容 bundle 则识别对应 bundle manifest。
3. **入口解析**：仅原生插件按插件目录或 `package.json#openclaw.extensions` 解析运行时入口，并执行边界检查。
4. **注册/归一化**：原生插件同步调用 `register(api)`；兼容 bundle 则把可承接的技能、命令、settings 等能力归一化进 registry，而不是导入任意运行时代码。
5. **激活**：所有原生插件注册与 bundle 归一化完成后，构建 `PluginRegistry`，初始化全局 Hook Runner。
...
> [!NOTE]
> Bundle 兼容性是选择性的：技能文件（如 `SKILL.md`）通常最容易映射；但涉及特定框架 API、运行时命令模型或平台专属 hooks 的部分，可能只会被部分加载，甚至被安全策略直接跳过。建议安装后通过 `openclaw plugins doctor` 与 `openclaw plugins list` 验证映射结果，确认哪些能力已成功加载、哪些被跳过。
