...
| **消息处理** | `message_received` | 观测型（即发即忘） | 消息到达后的旁路处理 |
...
观测型钩子（Void Hooks）通过 `runVoidHook()` **并行执行**（`Promise.all`），不能修改状态，用于审计与旁路处理。修改型钩子（Modifying Hooks）通过 `runModifyingHook()` **按优先级顺序执行**（高优先级先执行），每个钩子的修改结果流入下一个钩子，最终合并应用。此外还有两种同步钩子：`tool_result_persist`（热路径上链式修改持久化消息）和 `before_message_write`（JSONL 写入前的同步阻断），系统会检测并拒绝意外的异步处理器。

**全局 Hook Runner**（`hook-runner-global.ts`）：Gateway 启动时通过 `initializeGlobalHookRunner(registry)` 创建单例 Hook Runner，所有执行路径通过 `getGlobalHookRunner()` 获取。错误默认被捕获并记录（不会因单个插件崩溃而中断系统），通过 Proxy 模式实现延迟初始化。
