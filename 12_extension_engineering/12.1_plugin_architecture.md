...
1. **发现**：按 `plugins.load.paths` → workspace → global → bundled 的顺序扫描候选插件与兼容 bundle（first match wins）。
...
当前官方 CLI 支持多种市场来源：ClawHub、Claude 已知 marketplace 名称、本地 marketplace 根目录或 `marketplace.json`、GitHub 仓库简写，以及 git URL。写书时不应把某一种来源写成唯一官方形态。

安装方式：

```bash
# 从 ClawHub / npm 安装（bare spec 默认先查 ClawHub 再查 npm）
openclaw plugins install <插件包名>

# 显式指定 ClawHub 来源
openclaw plugins install clawhub:<插件包名>

# 从指定 marketplace 安装
openclaw plugins install <插件名> --marketplace <市场名或来源>
