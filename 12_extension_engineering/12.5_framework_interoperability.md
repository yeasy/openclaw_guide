...
Agent-to-Agent（A2A）协议定义了Agent间的通信规范，支持不同框架的Agent进行互操作。OpenClaw通过适配器模式实现与A2A协议的集成。
...
class CrewAIToOpenClawMigration:
    ...
    def migrate_agent(crewai_agent) -> Dict:
        ...
        return {
            ...
            “model”: “claude-3-5-sonnet-20250514”,  # 默认模型
            “tools”: []  # 需要手动映射
        }
