      ...
      groups: {
        '*': {
          toolsBySender: {
            ...
            'id:admin_user': { deny: ['group:runtime'] },
          },
        },
      },
    },
...
api.registerTool({
  ...
  async execute(id, params) {
    ...
    try {
      ...
      return {
        content: [{ type: "text", text: JSON.stringify({ success: false, error: "internal error", ticket_id: null }) }],
        isError: true,
      };
    }
  },
...
openclaw logs --follow --json \
  | jq -c 'select(.type=="log") | {time, level, subsystem, message, raw}'
```

版本策略与工具策略协同效果最佳，但要分清“平台能力”和“团队约定”：OpenClaw 不会自动理解 `query_database@1.1` 这类版本后缀。只有当你的团队确实把版本编码进 tool name，或通过插件发布流程维护并行工具名时，才谈得上按版本粒度允许/拒绝。
...
```javascript
// 下面更适合作为“插件内自定义编排 DSL”或 Lobster 工作流示意，
// 不是当前 OpenClaw 核心公开的 tools.orchestration schema。
{
  workflow: {
    id: 'deploy_service',
    steps: [
      { tool: 'validate_config', onFailure: 'abort' },
      { tool: 'build_image', requires: ['validate_config'] },
      { tool: 'push_registry', requires: ['build_image'] },
      { tool: 'deploy_canary', requires: ['push_registry'] },
    ],
  },
}
```
...
```bash
openclaw logs --follow --json \
  | jq -c 'select(.type=="log") | {time, subsystem, message, raw}'
```
