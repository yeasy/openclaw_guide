...
> [!NOTE]
> 上述三条命令为截至 2026 年 3 月官方 CLI 已稳定支持的自检命令。渠道与模型的探针式健康检查（如 `channels status --probe`、`models status --check`）可能因版本而异，请以 `openclaw --help` 输出与官方文档为准。
...
```bash
# 生成当前配置快照（使用固定文件名作为基线）
openclaw status --deep --format json > snapshots/config-baseline.json
openclaw plugins list --format json > snapshots/plugins-baseline.json
```

> [!NOTE]
> `openclaw status --deep --format json` 与 `openclaw plugins list --format json` 的 `--format` 参数需以实际 CLI 版本为准。若不支持 `--format json`，可通过 `openclaw logs --json` 结合 `jq` 提取等效信息。
...
diff <(jq -S '.tools.deny // []' "$BASELINE/config-baseline.json") \
     <(jq -S '.tools.deny // []' "$CURRENT/config-baseline.json") && \
  echo "PASS: Tool deny rules unchanged" || echo "FAIL: Tool deny rules changed"

echo "=== Plugin Snapshot Diff ==="
diff <(jq -S '.[] | {id, version, status}' "$BASELINE/plugins-baseline.json") \
     <(jq -S '.[] | {id, version, status}' "$CURRENT/plugins-baseline.json") && \
  echo "PASS: Plugin versions unchanged" || echo "FAIL: Plugin versions changed"
```
  ...
  verify:
    ...
    steps:
      ...
      - name: Run Snapshot Tests
        run: |
          openclaw status --deep --format json > /tmp/config-baseline.json
          openclaw plugins list --format json > /tmp/plugins-baseline.json
