...
> [!NOTE]
> 上述三条命令为官方 CLI 已稳定支持的自检命令。若还要继续确认渠道能力或模型 live 可用性，可再看 `channels capabilities`、`models status --probe`；相关参数与输出仍应以 `openclaw --help` 和官方文档为准。
...
openclaw logs --json | jq -c 'select(.type=="log") | {time, level, subsystem, message, raw}' \
...
#!/bin/bash
# 团队自定义快照对比脚本示例
# 用法：bash <your-snapshot-diff-script> ./snapshots ./snapshots-new
...
openclaw logs --follow --json \
  | jq -c 'select(.type=="log") | select((.message // "") | contains("<TRACE_ID>")) | {time, level, subsystem, message, raw}'
```
...
> [!NOTE]
> 上述流水线中的 `openclaw doctor`、`openclaw plugins doctor`、`openclaw status --deep`、`pnpm test:contracts:plugins`、`pnpm tool-display:check`、`pnpm test:changed` 都对应当前 OpenClaw 仓库里的命令。部署步骤（如灰度发布）因团队基础设施而异，此处仅展示验证阶段的参考结构。
...
```bash
#!/bin/bash
# 团队自定义集成测试脚本示例：通过日志验证工具调用行为，而非直接调用 CLI 子命令
...
# 不假设顶层存在固定字段；先筛出 JSON 日志，再从 message/raw 中找拒绝证据
if ! jq -c 'select(.type=="log") | {level, subsystem, message, raw}' "$LOG_FILE" | grep -Eiq "tool_denied|tool denied"; then
  echo "FAIL: No tool denial event found"
  kill $LOG_PID 2>/dev/null; exit 1
fi
