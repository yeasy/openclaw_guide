...
> Last verified: 2026-06-10. 本表用于维护 OpenClaw、模型、API、价格、协议和 workflow 相关高波动事实。

| 类别 | 当前维护口径 | 权威入口 | 编辑要求 |
| --- | --- | --- | --- |
| OpenClaw release | 安装命令、Node 要求、配置字段、CLI 行为以 OpenClaw release 和 docs 为准。 | OpenClaw docs / releases | 任何可复制命令都应能用当前版本复核。 |
| 模型与价格 | OpenAI、Claude、Gemini、本地模型价格与上下文只写 dated snapshot。 | [OpenAI Models](https://developers.openai.com/api/docs/models/all/), [Claude Models](https://platform.claude.com/docs/en/about-claude/models/overview), [Gemini Models](https://ai.google.dev/gemini-api/docs/models) | 预算模板必须标注假设和核验日期；Claude 侧“最强/旗舰”须区分 Fable 5（2026-06-09 起的全系最强广泛发布模型）与 Opus 4.8（Opus 档）。 |
