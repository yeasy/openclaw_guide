...
> [!WARNING]
> OpenClaw 使用日历版本号（CalVer），字段层级与命名可能随版本演进而变化。本附录仅提供迁移思路与当前 live 审计实例（2026.5.3）的可观察结构，不能替代官方文档与本地 `config schema`。
...
```text
2025.11.15  →  2026.1.9  →  2026.2.14  →  2026.3.30  →  2026.5.3
```
...
| Gateway | `gateway.port` / `gateway.auth.mode` / `controlUi.allowedOrigins` | 控制面鉴权与来源校验已经是显式结构 |
