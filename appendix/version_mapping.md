...
> [!WARNING]
> OpenClaw 使用日历版本号（CalVer），字段层级与命名可能随版本演进而变化。本附录仅提供迁移思路与当前 live 审计实例（2026.3.13）的可观察结构，不能替代官方文档与本地 `config.schema`。
...
```text
2025.11.15  →  2026.1.9  →  2026.2.14  →  2026.3.13
```
...
| 模型 | `model.primary` + `fallbacks` | 当前 live 实例优先暴露顺序回退链，而不是把 `models` 数组写成默认主结构 |
...
| 插件 | `plugins.entries.*` | 插件开关与配置已形成独立家族 |
...
| `agents.*.model` 单字符串 | `model.primary` + `fallbacks` | 把模型选择与回退链显式化 |
...
> `--repair` 与旧资料中的 `--fix` 常被混用。实际以本地 `openclaw doctor --help` 为准。
...
如果迁移涉及凭据存储，而不是普通字段层级调整，应使用 secrets 迁移命令：
...
3. 默认 agent 的 `model.primary` / `fallbacks` 是否仍指向可用模型。
