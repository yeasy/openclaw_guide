...
if command -v node >/dev/null 2>&1; then
  ...
  if [ "$node_major" -lt 22 ] || { [ "$node_major" -eq 22 ] && [ "$node_minor" -lt 19 ]; }; then
    echo "警告: OpenClaw 当前兼容线为 Node.js 22.19+；建议升级到 Node 24。"
  elif [ "$node_major" -eq 22 ]; then
    echo "提示: Node 22.19+ 受支持；生产、CI 和新安装推荐 Node 24。"
  fi
else
  echo "警告: 未安装 Node.js（推荐 Node 24；当前官方兼容线为 Node 22.19+）"
fi
...
| `警告: OpenClaw 当前兼容线为 Node.js 22.19+` | 当前 Node 版本过低 | 升级到 Node 24，或至少升级到官方兼容线以上 |
