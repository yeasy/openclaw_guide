...
该示例用于本地起步：打开本地模式，启用诊断能力，并保留默认智能体工作区。启动后用 `health/status` 验证，再用 Dashboard 进入 WebChat 做最小交互。
...
| `routing.rules` | 渠道侧 group policy、`agents.list[].groupChat.mentionPatterns` | 有效管理面逐渐从独立路由对象重定向到了渠道 Policy 与智能体群聊触发配置。 |
