...
- **用量观测与预算控制**（14.3）：使用 `/status`、`/cost`、`/compact` 等交互命令实时监控 Token 用量，结合 `session.reset` 与 `contextPruning` 配置控制上下文膨胀。
- **部署预算模板**（14.4）：个人场景月均 $30–90、团队场景 $500–3000、企业场景 $8000–50000，核心变量是模型选择和日均会话量。

### 优化检查清单

1. 是否已通过 `/cost` 或 `openclaw status --deep` 建立 Token 消耗基线？
2. 系统提示是否已精简到必要最小集？
