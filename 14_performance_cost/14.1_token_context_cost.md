...
OpenAI 的 GPT-5.5 / GPT-5.4 家族还可能按上下文长度、区域处理、Batch API 或缓存输入采用不同费率。上表只展示官方模型页的标准价格；生产估算时应按 OpenAI 当前 Pricing 页重新核对，尤其要注意 GPT-5.5 超过 272K 输入 token 时会进入更高价格层。
