...
| Claude Opus 4.7 | $5/1M | $25/1M | 最强推理，新 tokenizer | 高难度软件工程、复杂多步任务 |
...
> **注意：Claude Opus 4.7 的 Tokenizer 变化**：Opus 4.7 采用了新的 tokenizer，同样的文本可能产生 35% 更多的 token。虽然 per-token 定价保持不变（$5/$25），但实际有效成本可能显著上升。可通过启用 Prompt Caching（节省高达 90%）或 Batch Processing（节省 50%）来部分抵消这一成本增加。
