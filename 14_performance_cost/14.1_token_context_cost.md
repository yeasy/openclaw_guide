...
| Claude Opus 4.7 | $5/1M | $25/1M | 最强推理，新 tokenizer（+35% tokens），Adaptive Thinking | 高难度软件工程、复杂多步任务 |
...
OpenAI 的 GPT-5.5 / GPT-5.4 家族还区分标准与长上下文价格。上表只展示标准价格；当提示超过标准长上下文门槛时，应按官方 Pricing 页的 long-context tier 重新估算，例如 GPT-5.5 为 $10/$45、GPT-5.5 Pro 为 $60/$270、GPT-5.4 为 $5/$22.50、GPT-5.4 mini 为 $1.50/$6.75、GPT-5.4 nano 为 $0.40/$1.875（均为每 1M input/output tokens）。

典型分层：简单问题（80%）→ Haiku，中等（15%）→ Sonnet，复杂（5%）→ Opus，可降低平均成本 50-70%。

> **注意：Claude Opus 4.7 的 Tokenizer 变化与思考模式**：Opus 4.7 采用了新的 tokenizer，token 膨胀率因内容类型而异：英文纯文本接近 1.0x，代码和结构化数据可达 1.35x。虽然 per-token 定价保持不变（$5/$25），但复杂技术内容的实际有效成本可能显著上升。此外，Opus 4.7 不支持 Extended Thinking，仅支持 Adaptive Thinking（自动调整思考深度）。可通过启用 Prompt Caching（节省高达 90%）或 Batch Processing（节省 50%）来部分抵消成本增加。
