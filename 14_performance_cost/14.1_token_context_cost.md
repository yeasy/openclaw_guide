...
**成本对比**（Claude 侧为 2026-07-28 核验快照，OpenAI 侧仍为 2026 年 6 月写作时示例，均需以 [Anthropic Pricing](https://platform.claude.com/docs/en/about-claude/pricing) 与 [OpenAI Pricing](https://openai.com/api/pricing/) 为准）：
...
| GPT-5.5-pro | $30/1M | $180/1M | OpenAI pro 档（更多算力换更强推理）；GPT-5.6 起 pro 改为 Sol 的 `reasoning.mode: pro` | 顶级复杂推理、科研 |
| GPT-5.4 | $2.50/1M | $15/1M | OpenAI 较早一代通用主线 | 通用、复杂推理 |
...
> **注意：Claude 4.7 起的 Tokenizer 变化与思考模式**：[Anthropic 的 Opus 4.7 发布说明](https://www.anthropic.com/news/claude-opus-4-7)称，Opus 4.7 保持 $5/$25 per 1M tokens 定价，但新 tokenizer 会让同等输入约变为 1.0-1.35x tokens，具体取决于内容类型。[Anthropic 的 Extended Thinking 文档](https://platform.claude.com/docs/en/build-with-claude/extended-thinking)还说明，Opus 4.7 不再接受手动 `thinking: {type: "enabled", budget_tokens: N}`，应改用 Adaptive Thinking（`thinking: {type: "adaptive"}`）与 effort 参数。**这一 tokenizer 变化并不止于 Opus 4.7**：官方 Pricing 页说明 Claude 4.7 及之后的模型都使用该新 tokenizer，同等文本约多出 30% 的 token，而 Sonnet 4.6 及更早的模型仍使用旧 tokenizer。因此上表中的 Opus 5、Opus 4.8、Sonnet 5 与 Fable 5 同样受影响，按旧口径估算输入量会系统性偏低。可通过启用 Prompt Caching 或 Batch Processing 来部分抵消成本增加；真实账单仍需按当前官方 Pricing 页复核。
