...
|-----|---------|---------|--------|-----------|
| Claude 3 Haiku | $0.25/MTok | $1.25/MTok | 简单任务 | $0.005 |
| Claude 3 Sonnet | $3/MTok | $15/MTok | 中等复杂 | $0.08 |
| Claude 3 Opus | $15/MTok | $75/MTok | 复杂推理 | $0.40 |
| GPT-4 Turbo | $10/MTok | $30/MTok | 竞品对标 | $0.15 |

*注：MTok = 百万Token，平均成本基于平均消耗3000输入Token + 500输出Token*
...
class TokenBudgetManager {
  ...
  calculateCost(inputTokens, outputTokens, modelName) {
    const rates = {
      ...
      'claude-sonnet-3-5': { input: 3/1e6, output: 15/1e6 },
      'claude-haiku-3': { input: 0.25/1e6, output: 1.25/1e6 }
    };

    const rate = rates[modelName] || rates['claude-sonnet-3-5'];
  ...
  enableCostControlMode() {
    ...
    CONFIG.defaultModel = 'claude-haiku-3'; // 从Opus切到Haiku
