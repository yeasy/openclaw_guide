...
> **关于 Claude Opus 4.7 的成本估算**：如果使用 Opus 4.7（$5/$25/1M 的写作时快照），同样 2000 输入 + 500 输出 token 的基础成本为 `2000 × 0.000005 + 500 × 0.000025 = $0.0225`。若新 tokenizer 让输入 token 增加 35%，则约为 `$0.026`，还未计入 Adaptive Thinking 产生的额外思考 token。该段是估算模板，模型能力、价格与折扣应按当前官方 Pricing 页复核。
