...
**处置动作（Actions）**
1. **立即**：先保护系统边界
   - 先降入口流量：暂停低优先级渠道、降低并发、延迟非关键任务，并对新请求启用限流、排队上限或快速失败
   - 停止会放大故障的自动重试，优先遵守上游 `Retry-After` / 限流信号
...
3. **确认**：验证队列与副作用边界
   - 确认队列持久化状态、消费者积压量和死信队列，避免重启造成消息丢失
   - 核对工具调用、工单创建、支付/退款等副作用是否有幂等键，避免恢复时重复执行
   - 定位瓶颈在入口、队列、worker、模型供应商还是外部工具，不要用重启替代根因确认

4. **恢复**：带恢复计划重启
   ```bash
   # 仅在已限流、队列可安全 drain、幂等边界确认后，才按计划重启
...
诊断步骤：
1. 检查模型与认证探针：`openclaw models status --probe`
...
2. 验证修复：openclaw models status --probe
