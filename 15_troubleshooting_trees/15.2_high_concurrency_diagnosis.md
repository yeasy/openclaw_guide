...
```bash
# 查看最近日志中的 429 错误
openclaw logs --follow | grep "429"

# 检查网关速率限制状态
openclaw status --deep

# 获取性能指标（RPS、延迟、错误率）
openclaw health --json | jq '.metrics'
...
```bash
# 检查当前配置的 API 密钥
openclaw config | grep -A5 "api_key\|token"
...
# 查看最近的认证错误
openclaw logs --follow | grep "401\|403\|Unauthorized"
```
**处置动作（Actions）**
1. **立即**：重新验证凭证
   - 检查 API 密钥是否过期或已轮换
...
```bash
# 查看 Token 消耗统计
openclaw logs --json | jq '.cost'

# 获取聊天侧的本地成本摘要或 CLI 使用量视图
openclaw agent --message "/usage cost" 2>/dev/null
openclaw status --usage

# 分析哪些 Agent 消耗最多
openclaw logs --json | jq 'group_by(.agent_id) | map({agent: .[0].agent_id, total_tokens: map(.tokens) | add})'

# 检查当前模型配置（高成本vs低成本）
openclaw models list
openclaw config | grep -A2 "default_model"
```
**处置动作（Actions）**
1. **立即**：实施 Token 配给
...
2. **优化**：切换模型与提示
   ...
   # 或在 Agent 配置中设置成本目标
   # 编辑 ~/.openclaw/agents/<ID>/SOUL.md 或 USER.md
   ```
...
```bash
# 查看队列深度和消费速率
openclaw logs --follow --json | jq '.queue_depth, .processing_rate'

# 检查下游依赖服务（数据库、外部 API）的连接状态
openclaw status --deep | grep -A10 "connections\|pool"
...
# 如果怀疑内存泄漏，导出堆快照
openclaw sandbox explain --json | jq '.memory_usage'

# 查看是否有热点代码（CPU 高）
openclaw logs --follow | jq '.profile.hot_functions'
```
**处置动作（Actions）**
1. 若 **CPU 高**：
   - 启用 Profiling 找出热点代码
...
诊断步骤：
1. 检查请求模式：openclaw logs | grep "429"
2. 确认 API 配额：向供应商查询当前 RPS 限制
3. 检查其他消费者：是否有其他服务竞争同一配额

解决方案：
1. 立即：启动自适应限速器
   openclaw config edit  # 调整 rate_limiter.max_rps = 40
   openclaw restart
...
诊断步骤：
1. 检查 API 密钥状态：openclaw models auth status
...
2. 优化：为 Agent 设置 Token 上限
   编辑 ~/.openclaw/agents/<ID>/USER.md，添加 token_limit
...
当多个独立 Agent 在同一系统中运行时，它们会竞争有限的 Token 资源。详细的多 Agent Token 配额协调机制请参考第 7.4 章《多代理协作模式》与第 14.3 章《OpenClaw 的用量观测与预算控制》。

关键要点：
- 为每个 Agent 设置优先级（CRITICAL、HIGH、NORMAL、LOW）
- 分配日限额（基于历史消耗 × 1.3）和小时限额（日限额÷24×1.5）
- 监控 80% 和 90% 两个告警阈值
- 使用优先级队列进行动态资源分配
